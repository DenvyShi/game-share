...
function StatCompare_UpdateTargetFrames()
	...
	local anchorX = 0
	local alignMode = StatCompare_SaveVar and StatCompare_SaveVar.alignMode or "fixed"
	
	if alignMode == "fixed" and S_ItemTip_InspectFrame and S_ItemTip_InspectFrame:IsShown() then
		-- 如果装备列表显示，定位到装备列表右边
...
function STATCOMPARE_SlashCmdHandler(msg)
	...
	if ( (not msg) or (strlen(msg) <= 0 ) or (msg == "help") ) then
		...
		DEFAULT_CHAT_FRAME:AddMessage(GREEN.." /statc align fixed | free | lock"..CLOSE.." - 设置布局模式");
	...
	elseif (strsub(msg, 1, 5) == "align") then
		local param = strsub(msg, 7)
		param = string.gsub(param, "^%s*(.-)%s*$", "%1") -- chomp
		if param == "" then
			local currentMode = StatCompare_SaveVar and StatCompare_SaveVar.alignMode or "fixed"
			local modeText = ""
		    if currentMode == "fixed" then
				modeText = "默认"
			elseif currentMode == "free" then
				modeText = "自由"
			elseif currentMode == "locked" then
				modeText = "锁定"
			end
			DEFAULT_CHAT_FRAME:AddMessage(NORMAL.."当前布局模式: "..GREEN..modeText..CLOSE)
			DEFAULT_CHAT_FRAME:AddMessage(GREEN.." /statc align fixed "..CLOSE.."- 默认布局")
            DEFAULT_CHAT_FRAME:AddMessage(GREEN.." /statc align free  "..CLOSE.." - 自由布局")
            DEFAULT_CHAT_FRAME:AddMessage(GREEN.." /statc align lock  "..CLOSE.." - 锁定当前位置")
		elseif param == "fixed" then
			StatCompare_SaveVar.alignMode = "fixed"
			DEFAULT_CHAT_FRAME:AddMessage("StatCompare "..GREEN.."已设置为默认布局"..CLOSE)
		elseif (msg == "align free") then
			StatCompare_SaveVar.alignMode = "free"
			DEFAULT_CHAT_FRAME:AddMessage("StatCompare "..GREEN.."已设置为自由布局"..CLOSE)
		elseif msg == "align lock" then
			StatCompare_SaveVar.alignMode = "locked"
			StatCompare_SaveLockedPositions()
			DEFAULT_CHAT_FRAME:AddMessage("StatCompare "..GREEN.."已锁定当前的位置"..CLOSE)
		else
			DEFAULT_CHAT_FRAME:AddMessage("未知的参数: "..GREEN..param..CLOSE)
            DEFAULT_CHAT_FRAME:AddMessage("可用参数: fixed, free, lock")
		end
...
function StatCompare_OnEvent()
	if( event == "VARIABLES_LOADED" ) then
		...
		if StatCompare_SaveVar.alignMode == nil then
			if StatCompare_SaveVar.alignFree == nil then
				StatCompare_SaveVar.alignMode = "fixed"
			else
				if StatCompare_SaveVar.alignFree == true then
					StatCompare_SaveVar.alignMode = "free"
					StatCompare_SaveVar.alignFree = nil
				else
					StatCompare_SaveVar.alignMode = "fixed"
					StatCompare_SaveVar.alignFree = nil
				end
		...
		if not StatCompare_SaveVar.lockedPositions then
			StatCompare_SaveVar.lockedPositions = {}
		end
...
local function IsInspectingOther()
    if InspectFrame and InspectFrame:IsShown() then
        return true
    end
    
    if StatCompareTargetFrame and StatCompareTargetFrame:IsShown() then
        return true
    end
...
function SCShowFrame(frame,target,tiptitle,tiptext,anchorx,anchory)
	if StatCompare_IsDragging then
		return
	end
	--[[
	--debug
	DEFAULT_CHAT_FRAME:AddMessage("SCShowFrame: frame=" .. frame:GetName() .. 
        ", target=" .. target:GetName() .. 
        ", anchorx=" .. tostring(anchorx) .. 
        ", anchory=" .. tostring(anchory) ..
        ", inspectVisible=" .. tostring(inspectFrameVisible))
	--debug off
	]]
	...
	if alignMode == "free" then
		local savedPos = StatCompare_Player and StatCompare_Player["FramePositions"] and StatCompare_Player["FramePositions"][frameName]
		if savedPos and savedPos.point and savedPos.relativePoint then
			frame:SetPoint(savedPos.point, UIParent, savedPos.relativePoint, savedPos.xOfs or 0, savedPos.yOfs or 0)
		else
			frame:SetPoint("TOPLEFT", target, "TOPRIGHT", anchorx, anchory)
		end
	elseif alignMode == "locked" then

		local lockedPos = nil
		local frameName = frame:GetName()
		local isInspectingOther = IsInspectingOther()
		
		if isInspectingOther then
			lockedPos = StatCompare_SaveVar.lockedPositions and StatCompare_SaveVar.lockedPositions[frameName]
		else
			if frame == StatCompareSelfFrame then
				lockedPos = StatCompare_SaveVar.lockedPositionsSelfOnly and StatCompare_SaveVar.lockedPositionsSelfOnly[frameName]
			end
			if not lockedPos then
				lockedPos = StatCompare_SaveVar.lockedPositions and StatCompare_SaveVar.lockedPositions[frameName]
			end
		end
		if lockedPos and lockedPos.point and lockedPos.relativePoint then
			frame:SetPoint(lockedPos.point, UIParent, lockedPos.relativePoint, lockedPos.xOfs or 0, lockedPos.yOfs or 0)
		else
			frame:SetPoint("TOPLEFT", target, "TOPRIGHT", anchorx, anchory)
		end
	else --alignMode == "fixed"
		...
		if inspectFrameVisible then
			...
			if target:GetName() == "StatCompareTargetFrame" then
				...
				frame:SetPoint("TOPLEFT", S_ItemTip_InspectFrame, "TOPRIGHT", 0, 0);
			end
		else    -- 情况2: Inspect Frame 隐藏 - 按相同规则排布，但锚定到原始目标右侧
			if target:GetName() == "StatCompareTargetFrame" then
				-- SelfFrame 定位在 TargetFrame 的右侧
				frame:SetPoint("TOPLEFT", target, "TOPRIGHT", anchorx, anchory);
			else
				local xOffset = -35
				local yOffset = -13
				frame:SetPoint("TOPLEFT", target, "TOPRIGHT", xOffset, yOffset);
			end
		end
	end
	-- ========== 修改结束 ==========
        ...
        if elapsed >= 0.2 then  -- 延迟 0.2 秒
            ...
            if StatCompareSelfFrame and StatCompareSelfFrame:IsVisible() then
                ...
                if tiptext then
					...
					local anchorX = 200
					local anchorY = -12
                    if alignMode == "fixed" and isVisible and S_ItemTip_InspectFrame and S_ItemTip_InspectFrame:IsShown() then
                        anchorFrame = S_ItemTip_InspectFrame
						anchorX = 0
						anchorY = -2
                    end
...
function StatCompare_SaveLockedPositions()
    if not StatCompare_SaveVar then return end
    StatCompare_SaveVar.lockedPositions = StatCompare_SaveVar.lockedPositions or {}
	StatCompare_SaveVar.lockedPositionsSelfOnly = StatCompare_SaveVar.lockedPositionsSelfOnly or {}
    
    local function SaveFrameAbsolute(frame, key, storageTable)
        if not frame then return end
        ...
        if left and top then
			...
			local yFromTop = screenHeight - top
	...
	if isInspectingOther then
		SaveFrameAbsolute(StatCompareTargetFrame, "StatCompareTargetFrame", StatCompare_SaveVar.lockedPositions)
        SaveFrameAbsolute(StatCompareSelfFrame, "StatCompareSelfFrame", StatCompare_SaveVar.lockedPositions)
	else
		SaveFrameAbsolute(StatCompareSelfFrame, "StatCompareSelfFrame", StatCompare_SaveVar.lockedPositionsSelfOnly)
	end
end
