...
MacroPopupFrame:SetPoint("TOPLEFT", MacroFrame, "TOPRIGHT", -40, -8.5)
MacroEditButton:SetText("更改")
SetSize(MacroEditButton, 50, 25)
MacroEditButton:ClearAllPoints()
MacroEditButton:SetPoint("TOPLEFT", MacroFrameSelectedMacroBackground, 53, -30)
...
-- Addon frame object
local LargerMacroPopupFrame = CreateFrame("Frame", "LargerMacroPopupFrame")
LargerMacroPopupFrame:RegisterEvent("ADDON_LOADED")
LargerMacroPopupFrame:SetScript("OnEvent", function()
	if arg1 == "LargerMacroPopupFrame" then
		NUM_ICONS_PER_ROW = 5		--width
		NUM_ICON_ROWS = 7			--height
...
function LargerMacroPopupFrame:InitOnce()
	...
	LargerMacroPopupFrame.InitOnce = function() end
end

-- Initialization that should be called when the width/height values change
function LargerMacroPopupFrame:Init()
	...
	for i = 2, NUM_MACRO_ICONS_SHOWN do
		...
		a:ClearAllPoints()
		if (i - math.floor(i/NUM_ICONS_PER_ROW)*NUM_ICONS_PER_ROW) == 1 then
