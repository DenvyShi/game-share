...
local function OnMemberChange()
    ...
    if activity then
        ...
        if activity.unitname == Meeting.player then
            ...
            local target = activity.targetMembers or Meeting.GetActivityMaxMembers(activity.code)
...
function Meeting:OnCreate(id, code, comment, level, class, members, hc, classnum, targetMembers)
    -- 检查屏蔽词
    ...
    if item then
        ...
        item.targetMembers = targetMembers and tonumber(targetMembers) or nil
        if classnum then
        ...
        item = {
            ...
            targetMembers = targetMembers and tonumber(targetMembers) or nil,
