...
function Heart_SetupSettings()
	...
	if ((not Heart_Config["version"]) or (Heart_Config["version"] < 0.14001)) then
		...
		C_Print("Heart detect your old version of Heart. Will clean your classes sets for bug fixing. Sorry for the trouble.");
	end
	...
	if (not Heart_Config["rez_spell"]) then
		...
		Heart_Config["rez_spell"] = HEART_REZ_SPELL[C_my_enclass]
        end
        local rezspell = Heart_Config["rez_spell"]
        if rezspell and  Heart_Spells[rezspell]  then
        ...
        Heart_Config["rez_message"] = (fontOrange..C_my_name.." |r正在复活 $t ！")
...
function Heart_UpdateSettings()
	...
	if (not pre and not MRBCAT_LOADED ) then
		return
	end
...
function enemy_with_friendtarget()
	if IsFriendTargetofTarget then
		return IsFriendTargetofTarget()	
	end
	return nil	
end
