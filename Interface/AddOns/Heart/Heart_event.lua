...
function Heart_OnLoad()
	...
	C_my_class, C_my_enclass = UnitClass("player")
...
function Heart_OnEvent()
		...
		if (not Heart_supported_classes[C_my_class]) then
			...
			if WorldFrameMouseDown then
				WorldFrame:SetScript("OnMouseDown", function()
					Heart_MouseDown(); WorldFrameMouseDown();
				end)
			else
	...
	elseif (event == "SPELLCAST_DELAYED" and Heart_i_am_healing and arg1) then
		...
		Heart_healing[healingtarget][C_my_name]["TimeLeft"] = Heart_healing[healingtarget][C_my_name]["TimeLeft"] +
		arg1 / 1000
		SendAddonMessage("Heart",
			"Update, " ..
			healingtarget ..
			", " ..
			Heart_healing[healingtarget][C_my_name]["Spell"] ..
			", " ..
			Heart_healing[healingtarget][C_my_name]["Heal"] ..
			", " ..
			Heart_healing[healingtarget][C_my_name]["HealingOverTime"] ..
			", " ..
			Heart_healing[healingtarget][C_my_name]["CastTime"] ..
			", " .. Heart_healing[healingtarget][C_my_name]["TimeLeft"], "RAID")
	elseif (event == "SPELLCAST_FAILED" or event == "SPELLCAST_INTERRUPTED" or event == "SPELLCAST_STOP") then
		...
		SendAddonMessage("Heart",
			Heart_healing[healingtarget][C_my_name]["Status"] ..
			", " ..
			healingtarget ..
			", " ..
			Heart_healing[healingtarget][C_my_name]["Spell"] ..
	...
	elseif (event == "SPELLCAST_START" and arg2) then
		...
		Heart_healing[healingtarget][C_my_name]["TimeLeft"] = Heart_healing[healingtarget][C_my_name]["TimeLeft"] + arg2 -
		Heart_healing[healingtarget][C_my_name]["CastTime"]
	...
	elseif (msg == "tank") then
		...
		if (aka) then
			if (aka ~= "target" or UnitIsUnit("player", aka)) then
				...
				C_Print("单治疗目标已设置 (" .. aka .. "): " .. Heart_Tank["Name"])
			else
