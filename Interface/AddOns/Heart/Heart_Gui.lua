...
function Heart_TankButton_OnClick(arg1)
		...
		if (aka) then
		   if ( aka ~= "target" or UnitIsUnit("player", aka)) then
			...
			C_Print("指定专注治疗目标 (" ..aka.."): ".. Heart_Tank["Name"])
                   else
                        C_Print("专注目标必须是团队/小队成员: " .. C_UnitName(aka) .. " 他不是")
                        if (Heart_Tank and Heart_Tank["Name"]) then
                               C_Print("当前坦克剩余 (" ..Heart_Tank["Unit"].."): ".. Heart_Tank["Name"])
                        else
                               C_Print("米有专注治疗目标")
                        end
                   end
			...
			if (Heart_Tank and Heart_Tank["Name"]) then
				C_Print("移除专注治疗目标 " .. Heart_Tank["Name"])
                         else
				C_Print("没有专注治疗目标")
			end
			Heart_Tank = {
           ...
           if UnitExists("target") and Heart_CanRez("target") then
               ...
               for player, data in Heart_awaiting_rez do
                   if (data["is_dead"] and (not data["waiting_time"]) and (not Heart_GetOtherRezing(player))) then
                      ...
                      if Heart_Heal(player, rez_spell, Heart_Config["rez_rank"]) then
                         if (channel ~="NONE") then
                            if channel == "RAID" and (GetNumRaidMembers() <= 0) then
                               channel = "PARTY"
                            end
               ...
               CastSpell(Heart_Spells[rez_spell][Heart_Config["rez_rank"]]["ID"], BOOKTYPE_SPELL)
           end
      end
end
...
function Heart_UpdateHealCurrent(elapsed)
	...
	elseif (unit) then
		...
		if (not Heart_healing[healingtarget][C_my_name]["Complete"]) then
			...
			Heart_GUIHealCurrentAfterText:SetText("预估治疗 (" .. math.floor(afterhp / UnitHealthMax(unit) * 1000) / 10 .. "%)")
...
function Heart_UpdateScale()
         Heart_GUIHealCurrent:SetScale(Heart_Config["HealBars_Scale"] or 1)
         for a = 1, 15 do
            getglobal("Heart_GUIHealBars"..a):SetScale(Heart_Config["CastBar_Scale"] or 1)
         end
         Heart_ClickFrame:SetScale(Heart_Config["Buttons_Scale"] or 1)
end
...
function Heart_SetTV()
	if Heart_TargetView.UnitID then
		Heart_TargetView.UnitID=nil;
		--Heart_UpdateTVHealing();
	else
