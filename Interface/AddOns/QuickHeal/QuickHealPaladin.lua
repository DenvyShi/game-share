...
function QuickHeal_Paladin_GetRatioHealthyExplanation()
    ...
    if RatioHealthy >= RatioFull then
        return QUICKHEAL_SPELL_HOLY_LIGHT .. " 永远不会在战斗中使用. ";
    else
        if RatioHealthy > 0 then
            return QUICKHEAL_SPELL_HOLY_LIGHT .. " 只在战斗中使用， 如果目标超过 " .. RatioHealthy*100 .. "% 血量, 和只有当治疗效果大于最大的 " .. QUICKHEAL_SPELL_FLASH_OF_LIGHT .. " 时. ";          
        else
            return QUICKHEAL_SPELL_HOLY_LIGHT .. " 只在战斗中使用， 如果治愈效果比最大的 " .. QUICKHEAL_SPELL_FLASH_OF_LIGHT .. " 更大时. ";           
        end
    end
end
