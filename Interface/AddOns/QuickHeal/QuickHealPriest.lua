...
function QuickHealPohTest()
  ...
  QuickHealHealInfoFrameFontstring:SetFont("Fonts\\FZXHLJW.TTF", 9)
  QuickHealHealInfoFrameFontstring:SetText("我的文本")
...
function QuickHeal_Priest_GetRatioHealthyExplanation()
    if QuickHealVariables.RatioHealthyPriest >= QuickHealVariables.RatioFull then
        ...
        if QuickHealVariables.RatioHealthyPriest > 0 then
            ...
            return QUICKHEAL_SPELL_FLASH_HEAL .. " 永远不会被使用. " .. QUICKHEAL_SPELL_LESSER_HEAL .. ", " .. QUICKHEAL_SPELL_HEAL .. " 或 " .. QUICKHEAL_SPELL_GREATER_HEAL .. " 将永远在战斗中使用. ";
        end
    end
end
...
function QuickHeal_Priest_FindSpellToUse(Target)
    ...
    if quickHealHealMode == 1 then
      ...
      if not InCombat or TargetIsHealthy or maxRankFH<1 then
          ...
          QuickHeal_debug(string.format("不在战斗中或者目标是健康或者快速治疗术不可用, 将使用最近的低级治疗术,治疗术或者强效治疗术 (不使用快速治疗)"))
          ...
          QuickHeal_debug(string.format("在战斗中，和目标需要治疗和玩家有快速治疗, 将只会使用快速治疗"));
          if Health < QuickHealVariables.RatioFull then
