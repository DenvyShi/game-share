...
local DQHV = { -- Default values
    ...
    NotificationTextNormal = "治疗 %s , %s",
    NotificationTextWhisper = "治疗你 %s",
...
BINDING_NAME_QUICKHEAL_HEALMT = "治疗MT";
BINDING_NAME_QUICKHEAL_HEALNONMT = "治疗非MT";
BINDING_NAME_QUICKHEAL_HEALSELF = "治疗玩家（自己）";
...
BINDING_NAME_QUICKHEAL_QUICKHEAL_SETHEALMODE3 = "转换到3种治疗模式"
...
function TitanPanelQuickHealButton_OnLoad()
    this.registry = {
        ...
        tooltipTitle = QuickHealData.name .. " 配置",
...
local function Message(text,kind,duration)
    ...
    elseif kind == "Info" and QHV.MessageScreenCenterInfo then UIErrorsFrame:AddMessage(text, 1, 1, 1, 1, duration or 2)
    ...
    elseif kind == "Info" and QHV.MessageChatWindowInfo then writeLine(text, 1, 1, 1)
...
local function Initialise()
    ...
    if (myAddOnsFrame_Register) then
        myAddOnsFrame_Register(QuickHealData,{"重要的命令:\n'/qh cfg' 打开配置面板.\n'/qh help' 列出可用命令."});
    end
    ...
    elseif PlayerClass == "druid" then
        ...
        writeLine(QuickHealData.name .. " " .. QuickHealData.version .. " 不支持 " .. UnitClass('player') .. ". " .. QuickHealData.name .. " 所以不加载.")
        return;
    end
    ...
    writeLine(QuickHealData.name .. " " .. QuickHealData.version .. " for " .. UnitClass('player') .. " 加载成功. 请使用: '/qh help'.")
...
local function UpdateQuickHealOverhealStatus()
    ...
    -- Update the label
    local txt = floor(waste) .. "% 治疗会被浪费 (" .. floor(overheal) .. " 血量)";
    QuickHeal_debug(txt);
...
function NewUIErrorsFrame_OnEvent(...)
    ...
    if HealingTarget and event == "UI_ERROR_MESSAGE" and arg1 then
        if arg1 == ERR_SPELL_OUT_OF_RANGE then
            Message(string.format(SPELL_FAILED_OUT_OF_RANGE .. ". %s 被列入黑名单5秒.", UnitFullName(HealingTarget)),"Blacklist",5)
        ...
        elseif arg1 == SPELL_FAILED_LINE_OF_SIGHT then
            Message(string.format(SPELL_FAILED_LINE_OF_SIGHT .. ". %s 被列入黑名单2秒.", UnitFullName(HealingTarget)),"Blacklist",2)
            LastBlackListTime = GetTime();
...
function QuickHeal_ComboBoxNotificationStyle_Fill()
    UIDropDownMenu_AddButton{ text = "正常"; func = QuickHeal_ComboBoxNotificationStyle_Click; value = "NORMAL" };
    UIDropDownMenu_AddButton{ text = "玩家角色"; func = QuickHeal_ComboBoxNotificationStyle_Click; value = "RP" };
end
...
function QuickHeal_ComboBoxMessageConfigure_Fill()
    UIDropDownMenu_AddButton{ text = "治疗 (绿)"; func = QuickHeal_ComboBoxMessageConfigure_Click; value = "Healing" };
    UIDropDownMenu_AddButton{ text = "信息 (蓝)"; func = QuickHeal_ComboBoxMessageConfigure_Click; value = "Info" };
    UIDropDownMenu_AddButton{ text = "黑名单 (黄)"; func = QuickHeal_ComboBoxMessageConfigure_Click; value = "Blacklist" };
    UIDropDownMenu_AddButton{ text = "错误 (红)"; func = QuickHeal_ComboBoxMessageConfigure_Click; value = "Error" };
end
...
function QuickHeal_GetExplanation(Parameter)
    ...
    if Parameter == "RatioFull" then
        if QHV.RatioFull > 0 then
            return "只会治疗目标当小于 " .. QHV.RatioFull*100 .. "% 的血量时.";
        else
            return QuickHealData.name .. " 被禁用.";
        end
    end

    if Parameter == "RatioForceself" then
        if QHV.RatioForceself > 0 then
            return "如果你少于 " .. QHV.RatioForceself*100 .. "% 血量, 你将成为治疗的目标.";
        else
            return "自我保护禁用."
        end
    end
    ...
    if Parameter == "NotificationChannel" then
        if QHV.NotificationChannel then
            if QHV.NotificationChannelName and (QHV.NotificationChannelName ~= "") then
                return "通知将传送到频道 '" .. QHV.NotificationChannelName .. "'.";
            else
                return "输入一个频道名称，将通知发送给频道.";
            end
        else
            return "通知将不会传递到频道.";
        end
    ...
    if Parameter == "NotificationRaid" then
        if QHV.NotificationRaid then
            return "在RAID中，通知将被发送到RAID聊天";
        else
            return "在RAID中，通知将不会发送到RAID聊天.";
        end
    end
...
local function UnitIsHealable(unit,explain)
    if UnitExists(unit) then
        if EvaluateUnitCondition(unit, UnitIsFriend('player', unit), "不是一个好友",explain) then return false end
        if EvaluateUnitCondition(unit, not UnitIsEnemy(unit, 'player'), "是敌人",explain) then return false end
        if EvaluateUnitCondition(unit, not UnitCanAttack('player', unit), "可以被玩家攻击",explain) then return false end
        if EvaluateUnitCondition(unit, UnitIsConnected(unit), "没有连接",explain) then return false end
        if EvaluateUnitCondition(unit, not UnitIsDeadOrGhost(unit), "是死亡的或者灵魂",explain) then return false end
        if EvaluateUnitCondition(unit, UnitIsVisible(unit), "对客户端不可见",explain) then return false end
    else return false
    end
    return true
end
...
function QuickHeal_EstimateUnitHealNeed(unit,report)
    ...
    if report then QuickHeal_debug("血量亏损估计 (" .. Level .. " " .. string.lower(Class) .. " @ " .. HealthPercentage*100 .. "%)",HealNeed) end
    return HealNeed;
end
        ...
        if not RestrictSubgroup or RestrictParty or not InRaid() or (SubGroup and not QHV["FilterRaidGroup" .. SubGroup]) then
            if not IsBlacklisted(UnitFullName(unit)) then
                if SpellCanTargetUnit(unit) then                
                    ...
                    QuickHeal_debug(UnitFullName(unit) .. " (" .. unit .. ")","超出范围或无需治疗");
                end
            else
                QuickHeal_debug(UnitFullName(unit) .. " (" .. unit .. ")","是黑名单");
            end
        end
    end
...
local function ExecuteHeal(Target,SpellID)
    ...
    if UnitIsHealable('target') then
        ...
        if Target == 'targettarget' then
            ...
            QuickHeal_debug("可治疗目标预读, 暂时切换目标到目标的目标",old,'-->',UnitFullName('target'));
        end
        -- If healing target is not the current healable target clear the healable target
        if not (Target == 'target') then
            QuickHeal_debug("可治疗目标预读, 暂时清除目标",UnitFullName('target'));
            ClearTarget();
            TargetWasChanged = true;
        end
    end
    ...
    QuickHeal_debug("  施放: " .. SpellNameAndRank .. " 在 " .. UnitFullName(Target) .. " (" .. Target .. ")" .. ", ID: " .. SpellID);
    ...
    if SpellCanTargetUnit(Target) or ((Target == 'target') and HealingTarget) then
        ...
        if UnitIsUnit(Target,'player') then
            Message(string.format("施放 %s 给自己", SpellNameAndRank),"Healing",3)
        else
            Message(string.format("施放 %s 给 %s",SpellNameAndRank,UnitFullName(Target)),"Healing",3)
        end
    end
    ...
    if SpellIsTargeting() then 
        StopMonitor("法术不能施放给目标 " .. (UnitFullName(Target) or "unit"));
        SpellStopTargeting() 
    end
    ...
    if TargetWasChanged then
        ...
        TargetLastTarget();
        QuickHeal_debug("重新获得以前的目标",old,'-->',UnitFullName('target'));
    end
...
function QuickHeal(Target,SpellID,extParam)
    ...
    if QuickHealBusy then
        if HealingTarget and MassiveOverhealInProgress then
            QuickHeal_debug("大量过量治疗中止.");
            SpellStopCasting();
        else
            QuickHeal_debug("治疗进行中，忽略命令");
        end
        return;
    end
    ...
    if Target then -- Target is specified, check it
        ...
        if UnitIsHealable(Target,true) then
            ...
            if targetPercentage < QHV.RatioFull then
                ...
                if UnitIsUnit(Target,'player') then
                    Message("你不需要治疗","Info",2);
                elseif Target == 'target' then
                    Message(UnitFullName('target') .. " 不需要治疗","Info",2);
                elseif Target == "targettarget" then
                    Message(UnitFullName('target') .. "的目标 (" .. UnitFullName('targettarget') .. ") 不需要治疗","Info",2);
                else
                    Message(UnitFullName(Target) .. " 不需要治疗","Info",2);
                end
                SetCVar("autoSelfCast",AutoSelfCast);
            ...
            if Target == 'target' and not UnitExists('target') then
                Message("你没有一个目标","Error",2);
            elseif Target == 'targettarget' then
                if not UnitExists('target') then
                    Message("你没有一个目标","Error",2);
                elseif not UnitExists('targettarget') then
                    Message((UnitFullName('target') or "Target") .. " 没有目标","Error",2);
                else
                    Message(UnitFullName('target') .. "的目标 (" .. UnitFullName('targettarget') .. ") 无法治疗","Error",2);
                end
            elseif UnitExists(Target) then
                -- Unit exists but cannot be healed
                if UnitIsUnit(Target,'player') then
                    Message("你无法治疗","Error",2);
                else
                    Message(UnitFullName(Target) .. " 无法治疗","Error",2);
                end
            else
                Message("单位不存在","Error",2);
            end
            SetCVar("autoSelfCast",AutoSelfCast);
            QuickHealBusy = false;
            return;
        ...
        if not Target then -- No healing target found
            if Target == false then
                ...
                if Restrict == "mt" then
                    ...
                    if not tanks then
                        Message("没有任何玩家被队长指派为主力坦克","Error",2);
                    else
                        Message("没有主力坦克治疗","Info",2);
                    end
                elseif InParty() or InRaid() then
                    Message("没有人可以治疗","Info",2);
                else
                    Message("你不需要治疗","Info",2);
                end
            end
    ...
    -- Target acquired
    QuickHeal_debug(string.format("  治疗目标: %s (%s)", UnitFullName(Target), Target));
...
function QuickHeal_Command(msg)
    ...
    writeLine("/qh toggle - 在迅捷治疗和一般治疗中开关切换 (健康阈值 0% 或 100%)."); --hc05
    writeLine("/qh healmode [1,2,3]| hm [1,2,3] - 切换治疗模式 1, 2 或 3. 1: 默认; 2: 快速治疗 / 圣光闪现; 3: 最大HPS");
    writeLine("/qh downrank | dr - 打开 QuickHeal 的低级法术治疗等级滑块.");
    writeLine("/qh - 用最适合的治疗法术来治疗最需要的小队/团队队员.");
    writeLine("/qh player - 治疗自己.");
    writeLine("/qh target - 治疗目标.");
    writeLine("/qh targettarget - 治疗你当前目标的目标.");
    writeLine("/qh party - 在RAID中只治疗你的队伍.");
    writeLine("/qh mt - 治疗主坦克（只能在CTRaidAssist 或 oRA 里被定义的MT）");
    writeLine("/qh nonmt - 治疗非主坦克的所有目标（只能在CTRaidAssist 或 oRA 里被定义的MT之外的玩家）");
    writeLine("/qh subgroup - 治疗配置面板中选定的小组.");
    writeLine("/qh reset - 将配置重置为所有职业的默认参数.");
    writeLine("/qh healpct - 将优先考虑血量百分比最低的玩家.");
end
