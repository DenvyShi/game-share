...
function QuickHeal_Shaman_GetRatioHealthyExplanation()
    ...
    if RatioHealthy >= RatioFull then
        return QUICKHEAL_SPELL_HEALING_WAVE .. " 永远不会在战斗中使用. ";
    else
        if RatioHealthy > 0 then
            return QUICKHEAL_SPELL_HEALING_WAVE .. " 只会用于战斗(如果目标生命超过" .. RatioHealthy*100 .. "%), 并且只有当治疗结果大于最大等级的 " .. QUICKHEAL_SPELL_LESSER_HEALING_WAVE .. " 时. ";          
        else
            return QUICKHEAL_SPELL_HEALING_WAVE .. " 只会用于战斗,并且只有当治疗结果大于最大等级的" .. QUICKHEAL_SPELL_LESSER_HEALING_WAVE .. " 时. ";         
        end
    end
end
...
function QuickHeal_Shaman_FindSpellToUse(Target)
    ...
    if InCombat then
        ...
        debug(string.format("在战斗中，会优先次级治疗波"))
        ...
        debug(string.format("不在战斗中, 将使用最近可用的治疗波或次级治疗波"))
        if Health < RatioFull then
