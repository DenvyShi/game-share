...
function QuickHeal_Druid_GetRatioHealthyExplanation()
    ...
    if RatioHealthy >= RatioFull then
        ...
        if RatioHealthy > 0 then
            return QUICKHEAL_SPELL_REGROWTH .. " 会在战斗中使用,如果目标血量低于 " .. RatioHealthy*100 .. "%。否则就使用" .. QUICKHEAL_SPELL_HEALING_TOUCH .. ". ";
        else
...
function QuickHeal_Druid_FindSpellToUse(Target)
    ...
    debug("最终的治疗效果 (1.5,2.0,2.5,3.0,3.5,愈合)", healMod15,healMod20,healMod25,healMod30,healMod35,healModRG);
    ...
    if not InCombat or TargetIsHealthy or maxRankRG<1 then
        ...
        debug(string.format("不在战斗中或目标是健康的，或者愈合CD, 将使用治疗之触"))
        ...
        debug(string.format("在战斗中和目标不健康和愈合CD可用, 将使用愈合"));
        if Health < RatioFull then
