...
function DPSMate.Sync:OnLoad()
	...
	pid = DPSMate.Cache.DPSMateUser[player][1]
	Execute = {}
	...
	if DPSMate.DPSMateSettings["sync"] then
		...
		if time<30 then
			...
			elseif time>=3 and iterator==2 then
				self:HealingAllOut(DPSMate.Cache.DPSMateEHealing, "E")
				self:HealingAbilityOut(DPSMate.Cache.DPSMateEHealing, "E")
				self:HealingStatOut(DPSMate.Cache.DPSMateEHealing, "E")
				self:AbsorbsOut() 
				self:AbsorbsStatOut()
				iterator = 3
			...
			elseif time>=9 and iterator==4 then
				self:EDAllOut(DPSMate.Cache.DPSMateEDD, "D")
				self:EDAbilityOut(DPSMate.Cache.DPSMateEDD, "D")
				self:EDStatOut(DPSMate.Cache.DPSMateEDD, "D")
				iterator = 5
			elseif time>=12 and iterator==5 then
				self:HealingAllOut(DPSMate.Cache.DPSMateTHealing, "T")
				self:HealingAbilityOut(DPSMate.Cache.DPSMateTHealing, "T")
				self:HealingStatOut(DPSMate.Cache.DPSMateTHealing, "T")
			...
			elseif time>=18 and iterator==7 then
				self:HealingAllOut(DPSMate.Cache.DPSMateEHealingTaken, "ETaken")
				self:HealingTakenAbilityOut(DPSMate.Cache.DPSMateEHealingTaken, "E")
				self:HealingTakenStatOut(DPSMate.Cache.DPSMateEHealingTaken, "ETaken")
				self:HealingAllOut(DPSMate.Cache.DPSMateOverhealingTaken, "OTaken")
				self:HealingTakenAbilityOut(DPSMate.Cache.DPSMateOverhealingTaken, "O")
				self:HealingTakenStatOut(DPSMate.Cache.DPSMateOverhealingTaken, "OTaken")
				iterator = 8
			...
			elseif time>=24 and iterator==9 then
				self:HealingAllOut(DPSMate.Cache.DPSMateHealingTaken, "TTaken")
				self:HealingTakenAbilityOut(DPSMate.Cache.DPSMateHealingTaken, "T")
				self:HealingTakenStatOut(DPSMate.Cache.DPSMateHealingTaken, "TTaken")
				iterator = 10
			elseif time>=27 and iterator==10 then
				self:EDAllOut(DPSMate.Cache.DPSMateEDT, "T")
				self:EDAbilityOut(DPSMate.Cache.DPSMateEDT, "T")
				self:EDStatOut(DPSMate.Cache.DPSMateEDT, "T")
				iterator = 11
...
function DPSMate.Sync:ReceiveStartVote() 
	self:Participate()
	if DPSMate.DPSMateSettings["dataresetssync"] == 3 then
		DPSMate_Vote:Show()
	elseif DPSMate.DPSMateSettings["dataresetssync"] == 1 then
		self:Vote()
	end
end
...
function DPSMate.Sync:SyncStatus(arg2, arg4) 
	if arg2 == "0" then
		...
		if Arrays[1][usid] then DPSMate.Cache.DPSMateDamageDone[1][usid] = Arrays[1][usid] end
		if Arrays[2][usid] and ((DPSMate.ModuleNames["damagetaken"])) then DPSMate.Cache.DPSMateDamageTaken[1][usid] = Arrays[2][usid] else Arrays[2][usid] = nil end
		if  DPSMate.ModuleNames["enemydamagedone"] then
			for cat, val in pairs(Arrays[3]) do
				if val[usid] then
					if not DPSMate.Cache.DPSMateEDD[1][cat] then DPSMate.Cache.DPSMateEDD[1][cat] = {} end
					DPSMate.Cache.DPSMateEDD[1][cat][usid] = val[usid]
				end
			end
		else
			Arrays[3][usid] = nil
		...
		for cat, val in pairs(Arrays[4]) do
			if val[usid] then
				if not DPSMate.Cache.DPSMateEDT[1][cat] then DPSMate.Cache.DPSMateEDT[1][cat] = {} end
				DPSMate.Cache.DPSMateEDT[1][cat][usid] = val[usid]
			end
		end
		if Arrays[5][usid] and ( (DPSMate.ModuleNames["healing"])) then DPSMate.Cache.DPSMateTHealing[1][usid] = Arrays[5][usid] else Arrays[5][usid] = nil end
		if Arrays[6][usid] and ( (DPSMate.ModuleNames["effectivehealing"] or DPSMate.ModuleNames["healingandabsorbs"])) then DPSMate.Cache.DPSMateEHealing[1][usid] = Arrays[6][usid] else Arrays[6][usid] = nil end
		if Arrays[7][usid] and ( (DPSMate.ModuleNames["healingtaken"] or DPSMate.ModuleNames["healing"])) then DPSMate.Cache.DPSMateHealingTaken[1][usid] = Arrays[7][usid] else Arrays[7][usid] = nil end
		if Arrays[8][usid] and ( (DPSMate.ModuleNames["effectivehealingtaken"] or DPSMate.ModuleNames["effectivehealing"] or DPSMate.ModuleNames["healingandabsorbs"])) then DPSMate.Cache.DPSMateEHealingTaken[1][usid] = Arrays[8][usid] else Arrays[8][usid] = nil end
		...
		if Arrays[10][usid] and ( (DPSMate.ModuleNames["absorbs"] or DPSMate.ModuleNames["absorbstaken"] or DPSMate.ModuleNames["healingandabsorbs"])) then DPSMate.Cache.DPSMateAbsorbs[1][usid] = Arrays[10][usid] else Arrays[10][usid] = nil end
		if Arrays[11][usid] and ( (DPSMate.ModuleNames["deaths"])) then DPSMate.Cache.DPSMateDeaths[1][usid] = Arrays[11][usid] else Arrays[11][usid] = nil end
		if Arrays[12][usid] and ( (DPSMate.ModuleNames["interrupts"])) then DPSMate.Cache.DPSMateInterrupts[1][usid] = Arrays[12][usid] else Arrays[12][usid] = nil end
		if Arrays[13][usid] and ( (DPSMate.ModuleNames["decurses"] or DPSMate.ModuleNames["curepoison"] or DPSMate.ModuleNames["liftmagic"] or DPSMate.ModuleNames["curedisease"] or DPSMate.ModuleNames["dispels"])) then DPSMate.Cache.DPSMateDispels[1][usid] = Arrays[13][usid] else Arrays[13][usid] = nil end
		if Arrays[14][usid] and ( (DPSMate.ModuleNames["aurasgained"])) then DPSMate.Cache.DPSMateAurasGained[1][usid] = Arrays[14][usid] else Arrays[14][usid] = nil end
		if Arrays[15][usid] and ( (DPSMate.ModuleNames["overhealing"] or DPSMate.ModuleNames["OHealingTaken"])) then DPSMate.Cache.DPSMateOverhealingTaken[1][usid] = Arrays[15][usid] else Arrays[15][usid] = nil end
		if Arrays[16][usid] and ( (DPSMate.ModuleNames["threats"])) then DPSMate.Cache.DPSMateThreat[1][usid] = Arrays[16][usid] else Arrays[16][usid] = nil end
		DB.NeedUpdate = true
	else
...
function DPSMate.Sync:DMGDoneStatIn(arg2, arg4)
	...
	t[1] = tnbr(t[1])
	if t[1]>DPSMate.Cache.DPSMateCombatTime["total"] and (t[1]>5*DPSMate.Cache.DPSMateCombatTime["total"] or t[1]>20000) then return end
	if not Arrays[1][userid] then return end
	if not Arrays[1][userid][abid] then return end
	Arrays[1][userid][abid]["i"][t[1]]=tnbr(t[2])
	if t[1]>DPSMate.Cache.DPSMateCombatTime["total"] then DPSMate.Cache.DPSMateCombatTime["total"]=t[1] end
end
...
function DPSMate.Sync:DMGTakenAllIn(arg2, arg4)
	userid = DPSMate.Cache.DPSMateUser[arg4][1]
	t = {}
	strgsub(arg2, "(.-),", func)
	Arrays[2][userid] = {
		i = tnbr(t[1]),
...
function DPSMate.Sync:DMGTakenAbilityIn(arg2, arg4)
	t = {}
	strgsub(arg2, "(.-),", func)
	userid2, userid = DB:BuildUser(t[1]), DPSMate.Cache.DPSMateUser[arg4][1]
	if not Arrays[2][userid] then return end
...
function DPSMate.Sync:EDAllIn(arr, arg2, arg4)
	...
	Arrays[arr][userid][DPSMate.Cache.DPSMateUser[arg4][1]] = {
		i = tnbr(t[2]),
	}
end
...
function DPSMate.Sync:EDStatIn(arr, arg2, arg4)
	t = {}
	strgsub(arg2, "(.-),", func)
	userid, userid2, abid = DB:BuildUser(t[1]), DPSMate.Cache.DPSMateUser[arg4][1], DB:BuildAbility(t[4])
	...
	t[2] = tnbr(t[2])
	if t[2]>DPSMate.Cache.DPSMateCombatTime["total"] and (t[2]>5*DPSMate.Cache.DPSMateCombatTime["total"] or t[2]>20000) then return end
	Arrays[arr][userid][userid2][abid]["i"][t[2]] = tnbr(t[3])
	if t[2]>DPSMate.Cache.DPSMateCombatTime["total"] then DPSMate.Cache.DPSMateCombatTime["total"]=t[2] end
end

function DPSMate.Sync:EDAbilityIn(arr, arg2, arg4)
	t = {}
	strgsub(arg2, "(.-),", func)
	userid, userid2 = DB:BuildUser(t[1]), DPSMate.Cache.DPSMateUser[arg4][1]
	if not Arrays[arr][userid] then return end
	if not Arrays[arr][userid][userid2] then return end
...
function DPSMate.Sync:iAbsorbsIn(arg2, arg4) 
	t = {}
	strgsub(arg2, "(.-),", func)
	userid, userid2, userid3 = DPSMate.Cache.DPSMateUser[arg4][1], DB:BuildUser(t[1]), DB:BuildAbility(t[2])
...
function DPSMate.Sync:AbsorbsStatIn(arg2, arg4)
	t = {}
	strgsub(arg2, "(.-),", func)
	userid, userid2 = DPSMate.Cache.DPSMateUser[arg4][1], DB:BuildUser(t[1])
	if not DPSMate.Cache.DPSMateAbsorbs[1][userid] then return end
	if not DPSMate.Cache.DPSMateAbsorbs[1][userid][userid2] then return end
	t[2] = tnbr(t[2])
	if t[2]>DPSMate.Cache.DPSMateCombatTime["total"] and (t[2]>5*DPSMate.Cache.DPSMateCombatTime["total"] or t[2]>20000) then return end
	if t[4] then
...
function DPSMate.Sync:AbsorbsIn(arg2, arg4) 
	t = {}
	strgsub(arg2, "(.-),", func)
	userid, userid2, userid3, userid4 = DPSMate.Cache.DPSMateUser[arg4][1], DB:BuildUser(t[1]), DB:BuildAbility(t[2]), DB:BuildUser(t[4])
	local val = tnbr(t[3])
...
function DPSMate.Sync:InterruptsAllIn(arg2, arg4)
	t = {}
	strgsub(arg2, "(.-),", func)
	Arrays[12][DPSMate.Cache.DPSMateUser[arg4][1]] = {
		i = {
			[1] = tnbr(t[2]),
			[2] = {}
		},
...
function DPSMate.Sync:InterruptsAbilityIn(arg2, arg4)
	t = {}
	strgsub(arg2, "(.-),", func)
	userid, userid2, userid3 = DPSMate.Cache.DPSMateUser[arg4][1],DB:BuildAbility(t[1]), DB:BuildUser(t[2])
...
function DPSMate.Sync:DispelsIn(arg2, arg4)
	...
	strgsub(arg2, "(.-),", func)
	userid, userid2, userid3 = DPSMate.Cache.DPSMateUser[arg4][1], DB:BuildAbility(t[1]), DB:BuildUser(t[2])
	if not Arrays[13][userid] then Arrays[13][DPSMate.Cache.DPSMateUser[arg4][1]] = {i={[1]=0,[2]={}}} end
...
function DPSMate.Sync:AurasAllIn(arg2, arg4)
	t = {}
	strgsub(arg2, "(.-),", func)
	userid, userid2 = DPSMate.Cache.DPSMateUser[arg4][1], DB:BuildAbility(t[1])
...
function DPSMate.Sync:AurasStartEndIn(arg2, arg4, prefix)
	...
	if not Arrays[14][DPSMate.Cache.DPSMateUser[arg4][1]] then return end
	if not Arrays[14][DPSMate.Cache.DPSMateUser[arg4][1]][abid] then return end
	Arrays[14][DPSMate.Cache.DPSMateUser[arg4][1]][abid][prefix][tnbr(t[2])] = tnbr(t[3])
end

function DPSMate.Sync:AurasCauseIn(arg2, arg4)
	t = {}
	strgsub(arg2, "(.-),", func)
	userid, abid = DPSMate.Cache.DPSMateUser[arg4][1], DB:BuildAbility(t[1])
...
function DPSMate.Sync:DMGDoneAllOut()
	if DPSMate.Cache.DPSMateDamageDone[1][pid] then
		Buffer[cou] = {"DPSMate_DMGDoneAll", DPSMate.Cache.DPSMateDamageDone[1][pid]["i"]..","}
		cou = cou + 1
	end
end

function DPSMate.Sync:DMGDoneStatOut()
	if not DPSMate.Cache.DPSMateDamageDone[1][pid] then return end
	local ab, arr, i
	for cat, val in pairs(DPSMate.Cache.DPSMateDamageDone[1][pid]) do
...
function DPSMate.Sync:DMGTakenAllOut()
	if DPSMate.Cache.DPSMateDamageTaken[1][pid] then
		Buffer[cou] = {"DPSMate_DMGTakenAll", DPSMate.Cache.DPSMateDamageTaken[1][pid]["i"]..","}
		cou = cou + 1
	end
end

function DPSMate.Sync:DMGTakenStatOut()
	if not DPSMate.Cache.DPSMateDamageTaken[1][pid] then return end
	local enemy, ab, arr, i
	for cat, val in pairs(DPSMate.Cache.DPSMateDamageTaken[1][pid]) do
		if cat~="i" then
...
function DPSMate.Sync:AbsorbsOut() 
	if not DPSMate.Cache.DPSMateAbsorbs[1][pid] then return end
	local owner, ab
	for cat, val in pairs(DPSMate.Cache.DPSMateAbsorbs[1][pid]) do -- owner
...
function DPSMate.Sync:DeathsAllOut()
	if not DPSMate.Cache.DPSMateDeaths[1][pid] then return end
	for cat, val in pairs(DPSMate.Cache.DPSMateDeaths[1][pid]) do -- death count
...
function DPSMate.Sync:InterruptsAllOut()
	if DPSMate.Cache.DPSMateInterrupts[1][pid] then
		Buffer[cou] = {"DPSMate_InterruptsAll", playerclass..","..DPSMate.Cache.DPSMateInterrupts[1][pid]["i"][1]..","}
		cou = cou + 1
	end
end

function DPSMate.Sync:InterruptsAbilityOut()
	if not DPSMate.Cache.DPSMateInterrupts[1][pid] then return end
	for cat, val in pairs(DPSMate.Cache.DPSMateInterrupts[1][pid]) do
		if cat~="i" then
			for ca, va in pairs(val) do
...
function DPSMate.Sync:DispelsOut()
	if not DPSMate.Cache.DPSMateDispels[1][pid] then return end
	local ab, user
	for cat, val in pairs(DPSMate.Cache.DPSMateDispels[1][pid]) do -- Ability
		if cat=="i" then
...
function DPSMate.Sync:AurasOut()
	if not DPSMate.Cache.DPSMateAurasGained[1][pid] then return end
	local p = 0
	local ability
	for cat, val in pairs(DPSMate.Cache.DPSMateAurasGained[1][pid]) do -- ability
...
function DPSMate.Sync:ThreatOut()
	if not DPSMate.Cache.DPSMateThreat[1][pid] then return end
	local tar
	for cat, val in pairs(DPSMate.Cache.DPSMateThreat[1][pid]) do
...
function DPSMate.Sync:ThreatStatsIn(arg2,arg4)
	...
	if t[3]>DPSMate.Cache.DPSMateCombatTime["total"] then DPSMate.Cache.DPSMateCombatTime["total"]=t[3] end
end
