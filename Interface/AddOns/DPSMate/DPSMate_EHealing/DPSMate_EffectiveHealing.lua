...
DPSMate.Options.Options[1]["args"]["effectivehealing"] = {
	...
	get = function() return DPSMate.DPSMateSettings["windows"][DPSMate.Options.Dewdrop:GetOpenedParent().Key]["options"][1]["effectivehealing"] end,
...
function DPSMate.Modules.EffectiveHealing:GetSettingValues(arr, cbt, k,ecbt)
	...
	for cat, val in pairs(sortedTable) do
		...
		if DPSMate.DPSMateSettings["columnsehealing"][1] then str[1] = " "..DPSMate:Commas(va, k)..p; strt[2] = DPSMate:Commas(tot, k)..pt end
		if DPSMate.DPSMateSettings["columnsehealing"][3] then str[2] = " ("..strformat("%.1f", 100*varea/totr).."%)" end
		if DPSMate.DPSMateSettings["columnsehealing"][2] then str[3] = "("..strformat("%.1f", va/cbt)..")"; strt[1] = "("..strformat("%.1f", (tot/cbt))..pt..")" end
		if DPSMate.DPSMateSettings["columnsehealing"][4] then str[4] = " ("..strformat("%.1f", va/(ecbt[pname] or cbt))..p..")" end
		tinsert(name, pname) 
...
function DPSMate.Modules.EffectiveHealing:ShowTooltip(user, k)
	if DPSMate.DPSMateSettings["informativetooltips"] then
		local a,b,c = DPSMate.Modules.EffectiveHealing:EvalTable(DPSMate.Cache.DPSMateUser[user], k)
		local db = DPSMate:GetModeByArr(DPSMate.Cache.DPSMateEHealingTaken, k, "EHealingTaken")
		local abn, p, i = {}, 1, 1
		
		for cat, val in pairs(db) do
			if val[DPSMate.Cache.DPSMateUser[user][1]] then
				p = 0
				for _, va in pairs(val[DPSMate.Cache.DPSMateUser[user][1]]) do
					p = p + va[1]
				end
				if p>0 then
					i = 1
		...
		GameTooltip:AddLine(DPSMate.L["tttop"]..DPSMate.DPSMateSettings["subviewrows"]..DPSMate.L["tthealing"]..DPSMate.L["ttabilities"])
		...
		GameTooltip:AddLine(DPSMate.L["tttop"]..DPSMate.DPSMateSettings["subviewrows"]..DPSMate.L["tthealed"])
		for i=1, DPSMate.DPSMateSettings["subviewrows"] do
