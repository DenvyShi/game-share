...
DPSMate.Options.Options[1]["args"]["effectivehps"] = {
	...
	get = function() return DPSMate.DPSMateSettings["windows"][DPSMate.Options.Dewdrop:GetOpenedParent().Key]["options"][1]["effectivehps"] end,
...
function DPSMate.Modules.EffectiveHPS:GetSettingValues(arr, cbt, k,ecbt)
	...
	for cat, val in pairs(sortedTable) do
		...
		if DPSMate.DPSMateSettings["columnsehps"][2] then str[1] = " "..strformat("%.1f", va/cbt)..p; strt[2] = " "..strformat("%.1f", (tot/cbt))..pt end
		if DPSMate.DPSMateSettings["columnsehps"][3] then str[2] = " ("..strformat("%.1f", 100*varea/totr).."%)" end
		if DPSMate.DPSMateSettings["columnsehps"][1] then str[3] = "("..DPSMate:Commas(va, k)..p..")"; strt[1] = "("..DPSMate:Commas(tot, k)..pt..")" end
		if DPSMate.DPSMateSettings["columnsehps"][4] then str[4] = " ("..strformat("%.1f", va/(ecbt[pname] or cbt))..p..")" end
		tinsert(name, pname) 
...
function DPSMate.Modules.EffectiveHPS:ShowTooltip(user, k)
	if DPSMate.DPSMateSettings["informativetooltips"] then
		local a,b,c = DPSMate.Modules.EffectiveHPS:EvalTable(DPSMate.Cache.DPSMateUser[user], k)
		local db, cbt = DPSMate:GetModeByArr(DPSMate.Cache.DPSMateEHealingTaken, k, "EHealingTaken")
		local abn, p, i = {}, 1, 1
