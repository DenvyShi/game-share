...
function DPSMate.Modules.DetailsDamage:UpdateDetails(obj, key)
	...
	db2 = DPSMate:GetModeByArr(DPSMate.Cache.DPSMateEDT, key, "EDTaken")
	...
	DetailsArr, DetailsTotal, DmgArr = DPSMate.Modules.Damage:EvalTable(DPSMate.Cache.DPSMateUser[DetailsUser], curKey)
	...
	if not g then
		...
		g2=DPSMate.Options.graph:CreateGraphLine("DMGLineGraph",DPSMate_Details_DiagramLine,"CENTER","CENTER",0,0,950,230)
	...
	DPSMate_Details:SetScale((DPSMate.DPSMateSettings["targetscale"] or 0.58)/UIParent:GetScale())
...
function DPSMate.Modules.DetailsDamage:UpdateCompare(obj, key, comp)
	...
	DPSMate_Details_CompareDamage_SubTitle:SetText(DPSMate.L["activity"]..strformat("%.2f", DPSMate.Cache.DPSMateCombatTime["effective"][key][comp] or 0).."s "..DPSMate.L["of"].." "..strformat("%.2f", cbt).."s ("..strformat("%.2f", 100*(DPSMate.Cache.DPSMateCombatTime["effective"][key][comp] or 0)/cbt).."%)")
	...
	DetailsArrComp, DetailsTotalComp, DmgArrComp = DPSMate.RegistredModules[DPSMate.DPSMateSettings["windows"][curKey]["CurMode"]]:EvalTable(DPSMate.Cache.DPSMateUser[comp], curKey)
	...
	if not g4 then
		...
		g5=DPSMate.Options.graph:CreateGraphLine("DMGLineGraphComp",DPSMate_Details_CompareDamage_DiagramLine,"CENTER","CENTER",0,0,950,230)
		...
		g6:SetXLabels(true)
		g7=DPSMate.Options.graph:CreateGraphLine("DMGLineGraphSum",DPSMate_Details_CompareDamage_Graph,"CENTER","CENTER",0,0,1950,230)
	end
...
function DPSMate.Modules.DetailsDamage:EvalToggleTable(comp)
	...
	for cat, val in pairs(db2) do
		if val[DPSMate.Cache.DPSMateUser[comp or DetailsUser][1]] then
			local c = {[1] = 0,[2] = {},[3] = {}}
			for p, v in pairs(val[DPSMate.Cache.DPSMateUser[comp or DetailsUser][1]]) do
				if p ~= "i" then
					local i = 1
					while true do
						if (not c[2][i]) then
			...
			c[1] = val[DPSMate.Cache.DPSMateUser[comp or DetailsUser][1]]["i"]
			-- pet
			local user = comp or DetailsUser
			...
			if petsname and DPSMate.DPSMateSettings["mergepets"] then
				for petname,_ in pairs(petsname) do
					if petname ~= user then
						if val[DPSMate.Cache.DPSMateUser[petname][1]] then
							for p, v in pairs(val[DPSMate.Cache.DPSMateUser[petname][1]]) do
								if p ~= "i" then
									local i = 1
									while true do
										if (not c[2][i]) then
							...
							c[1] = c[1] + val[DPSMate.Cache.DPSMateUser[petname][1]]["i"]
						end
					end
				end
			end
...
function DPSMate.Modules.DetailsDamage:ScrollFrame_Update(comp)
	...
	for line=1,10 do
		...
		if uArr[lineplusoffset] ~= nil then
			...
			_G(path.."_ScrollButton"..line.."_Value"):SetText(dArr[lineplusoffset][1].." ("..strformat("%.1f", (dArr[lineplusoffset][1]*100/dTot)).."%)")
			...
			if len < 10 then
				_G(path.."_ScrollButton"..line):SetWidth(335)
				_G(path.."_ScrollButton"..line.."_Name"):SetWidth(225)
			else
				_G(path.."_ScrollButton"..line):SetWidth(320)
				_G(path.."_ScrollButton"..line.."_Name"):SetWidth(210)
			end
...
function DPSMate.Modules.DetailsDamage:Player_Update(comp)
	...
	for line=1,8 do
		...
		if d1[lineplusoffset] ~= nil then
			...
			_G(path.."_ScrollButton"..line.."_Value"):SetText(d2[lineplusoffset][1].." ("..strformat("%.1f", (d2[lineplusoffset][1]*100/d3)).."%)")
...
function DPSMate.Modules.DetailsDamage:PlayerSpells_Update(i, comp)
	...
	local offset = FauxScrollFrame_GetOffset(_G("DPSMate_Details"..comp.."_player_ScrollFrame"))
	obj.id = (i and  (i + offset)) or obj.id
	...
	local len = DPSMate:TableLength(d2[obj.id][2])
	local coeff = len-10
	if not obj.oset or obj.oset<0 then
		obj.oset = 0
	end
	...
	for line=1,10 do
		...
		if d2[obj.id][2][lineplusoffset] ~= nil then
			...
			_G(path.."_ScrollButton"..line.."_Value"):SetText(d2[obj.id][3][lineplusoffset][13].." ("..strformat("%.1f", (d2[obj.id][3][lineplusoffset][13]*100/d2[obj.id][1])).."%)")
			_G(path.."_ScrollButton"..line.."_Icon"):SetTexture(DPSMate.BabbleSpell:GetSpellIcon(strsub(ability, 1, (strfind(ability, "%(") or 0)-1) or ability))
			if len < 10 then
				_G(path.."_ScrollButton"..line):SetWidth(335)
				_G(path.."_ScrollButton"..line.."_Name"):SetWidth(225)
	...
	for p=1, 8 do
		if obj.id - offset == p then
			_G("DPSMate_Details"..comp.."_player_ScrollButton"..p.."_selected"):Show()
		else
...
function DPSMate.Modules.DetailsDamage:SelectDetailsButton(i, comp, cname)
	...
	local user = DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]
	if toggle then
		...
		local PetId = dArr[lineplusoffset][2] or nil
		user=DPSMate.Cache.DPSMateUser[cname or DetailsUser][1];

		if PetId then
			local petname = DPSMate:GetUserById(PetId)
			if petname and petname~=(cname or DetailsUser) then 
				user = PetId
			end
		end
...
function DPSMate.Modules.DetailsDamage:UpdatePie(gg, cname)
	...
	for cat, val in uArr do
		local user
		if (dArr[cat][2]) and DPSMate.DPSMateSettings["mergepets"] and DPSMate:GetUserById(dArr[cat][2]) ~= (cname or DetailsUser) then 
			user=dArr[cat][2]
		else 
			user=DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]
		end
		local percent = (db[user][val][13]*100/dTot)
		gg:AddPie(percent, 0, DPSMate:GetUserById(user)..": "..DPSMate:GetAbilityById(val))
	end
end
...
function DPSMate.Modules.DetailsDamage:UpdateStackedGraph(gg, comp, cname)
	...
	local temp2 = {}
	if toggle3 then
		for cat, val in pairs(db2[d1[d4]][DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]]) do
		...
		if petsname and DPSMate.DPSMateSettings["mergepets"] then
			for petname,_ in pairs(petsname) do 
				if petname ~= user then
					local PetId = DPSMate.Cache.DPSMateUser[petname][1]
					if db2[d1[d4]][PetId] then
						for cat, val in pairs(db2[d1[d4]][PetId]) do
							if cat~="i" and val["i"] then
								for c, v in pairs(val["i"]) do
									local key = tonumber(strformat("%.1f", c))
									if not temp[cat] then
		...
		for cat, val in pairs(db[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]]) do
			if cat~="i" and val["i"] then
				local temp = {}
				for c, v in pairs(val["i"]) do
					local key = tonumber(strformat("%.1f", c))
		...
		if petsname and DPSMate.DPSMateSettings["mergepets"] then 
			for petname,_ in pairs(petsname) do 
				if petname ~= user then
					local PetId = DPSMate.Cache.DPSMateUser[petname][1]
					if db[PetId] then
						for cat, val in pairs(db[PetId]) do
							if cat~="i" and val["i"] then
								local temp = {}
								for c, v in pairs(val["i"]) do
									local key = tonumber(strformat("%.1f", c))
...
function DPSMate.Modules.DetailsDamage:CreateGraphTable(comp)
	...
	for i=1, 8 do
		-- Horizontal
		lines[i] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details"..comp.."_LogDetails"), 5, 270-i*30, 376, 270-i*30, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
		lines[i]:Show()
	end
	-- Vertical
	lines[9] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details"..comp.."_LogDetails"), 57, 265, 57, 2, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[9]:Show()
	
	lines[10] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details"..comp.."_LogDetails"), 192, 265, 192, 2, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[10]:Show()
	
	lines[11] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details"..comp.."_LogDetails"), 252, 265, 252, 2, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[11]:Show()
	
	lines[12] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details"..comp.."_LogDetails"), 312, 265, 312, 2, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[12]:Show()
end
...
function DPSMate.Modules.DetailsDamage:ProcsDropDown()
	...
	if arr[DPSMate.Cache.DPSMateUser[DetailsUser][1]] then
		for cat, val in pairs(arr[DPSMate.Cache.DPSMateUser[DetailsUser][1]]) do
...
function DPSMate.Modules.DetailsDamage:ProcsDropDown_CompareDamage()
	...
	-- Adding dynamic channel
	if arr[DPSMate.Cache.DPSMateUser[DetailsUserComp][1]] then
		for cat, val in pairs(arr[DPSMate.Cache.DPSMateUser[DetailsUserComp][1]]) do
			local ability = DPSMate:GetAbilityById(cat)
			if DPSMate.Parser.procs[ability] or DPSMate.Parser.DmgProcs[ability] then
				UIDropDownMenu_AddButton{
					text = ability,
...
function DPSMate.Modules.DetailsDamage:SortLineTable(t, b, cname)
	local newArr = {}
	if b then
		for cat, val in t[b][DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]] do
			if cat~="i" and val["i"] then
		...
		for cat, val in t[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]] do
			if cat~="i" and val["i"] then
				for ca, va in val["i"] do
					local i=1
					while true do
	...
	for cat, val in pairs(DPSMate.DPSMateSettings["windows"][k]["options"][2]) do
		if val then
			if strfind(cat, "segment") then
				...
				return DPSMate.DPSMateHistory["Auras"][num]
			else
				return DPSMate.Cache.DPSMateAurasGained[modes[cat]]
			end
		end
	end
end
...
function DPSMate.Modules.DetailsDamage:CheckProcs(name, val, cname)
	local arr = DPSMate.Modules.DetailsDamage:GetAuraGainedArr(curKey)
	if arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]] then
		if arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name] then
			for i=1, DPSMate:TableLength(arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name][1]) do
				if not arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name][1][i] or not arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name][2][i] or arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name][4] then return false end
				if val > arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name][1][i] and val < arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name][2][i] then
					return true
				end
			end
		end
...
function DPSMate.Modules.DetailsDamage:AddProcPoints(name, dat, cname)
	...
	if arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]] then
		if arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name] then
			if arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name][4] then
				for cat, val in pairs(dat) do
					for i=1, DPSMate:TableLength(arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name][1]) do
						if arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name][1][i]<=val[1] then
							local tempbool = true
							for _, va in pairs(data) do
								if va[1] == arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name][1][i] then
									tempbool = false
									break
								end
							end
							if tempbool then	
								bool = true
								tinsert(data, {arr[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][name][1][i], LastVal, {val[1], val[2]}})
							end
						end
					end
					LastVal = {val[1], val[2]}
