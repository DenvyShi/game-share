...
function DPSMate.Modules.DetailsHealing:UpdateDetails(obj, key)
	...
	db2 = DPSMate:GetModeByArr(DPSMate.Cache.DPSMateHealingTaken, key, "THealingTaken")
	...
	DPSMate_Details_Healing_SubTitle:SetText(DPSMate.L["activity"]..strformat("%.1f", (ecbt[obj.user] or 0)+1).."s "..DPSMate.L["of"].." "..strformat("%.1f", cbt).."s ("..strformat("%.1f", 100*((ecbt[obj.user] or 0)+1)/cbt).."%)")
	...
	DetailsArr, DetailsTotal, DmgArr = DPSMate.Modules.Healing:EvalTable(DPSMate.Cache.DPSMateUser[DetailsUser], curKey)
	...
	if not g then
		...
		g2=DPSMate.Options.graph:CreateGraphLine("RHLineGraph",DPSMate_Details_Healing_DiagramLine,"CENTER","CENTER",0,0,950,230)
		g3=DPSMate.Options.graph:CreateStackedGraph("RHStackedGraph",DPSMate_Details_Healing_DiagramLine,"CENTER","CENTER",0,0,950,230)
	...
	DPSMate_Details_Healing:SetScale((DPSMate.DPSMateSettings["targetscale"] or 0.58)/UIParent:GetScale())
end
...
function DPSMate.Modules.DetailsHealing:UpdateCompare(obj, key, comp)
	...
	DPSMate_Details_CompareHealing_SubTitle:SetText(DPSMate.L["activity"]..strformat("%.1f", DPSMate.Cache.DPSMateCombatTime["effective"][key][comp] or 0).."s "..DPSMate.L["of"].." "..strformat("%.1f", cbt).."s ("..strformat("%.1f", 100*(DPSMate.Cache.DPSMateCombatTime["effective"][key][comp] or 0)/cbt).."%)")
	...
	if not g4 then
		...
		g5=DPSMate.Options.graph:CreateGraphLine("RHLineGraphComp",DPSMate_Details_CompareHealing_DiagramLine,"CENTER","CENTER",0,0,950,230)
		...
		g6:SetXLabels(true)
		g7=DPSMate.Options.graph:CreateGraphLine("RHLineGraphSum",DPSMate_Details_CompareHealing_Graph,"CENTER","CENTER",0,0,1950,230)
	end
...
function DPSMate.Modules.DetailsHealing:EvalToggleTable(cname)
	...
	for cat, val in db2 do
		if val[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]] then
			local CV = 0
			local c = {[1] = 0,[2] = {},[3] = {}}
			for p, v in val[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]] do
				CV = CV + v[1]
				local i = 1
				while true do
					if (not c[2][i]) then
...
function DPSMate.Modules.DetailsHealing:ScrollFrame_Update(comp)
	...
	for line=1,10 do
		...
		if uArr[lineplusoffset] ~= nil then
			...
			_G("DPSMate_Details_"..comp.."Healing_Log_ScrollButton"..line.."_Value"):SetText(dArr[lineplusoffset].." ("..strformat("%.1f", (dArr[lineplusoffset]*100/dTot)).."%)")
			...
			if len < 10 then
				_G("DPSMate_Details_"..comp.."Healing_Log_ScrollButton"..line):SetWidth(335)
				_G("DPSMate_Details_"..comp.."Healing_Log_ScrollButton"..line.."_Name"):SetWidth(225)
			else
				_G("DPSMate_Details_"..comp.."Healing_Log_ScrollButton"..line):SetWidth(320)
				_G("DPSMate_Details_"..comp.."Healing_Log_ScrollButton"..line.."_Name"):SetWidth(210)
			end
...
function DPSMate.Modules.DetailsHealing:SelectDetails_HealingButton(i, comp, cname)
	...
	local user, pet = DPSMate.Cache.DPSMateUser[cname or DetailsUser][1], ""
	if toggle then
...
function DPSMate.Modules.DetailsHealing:Player_Update(comp)
	...
	for line=1,8 do
		...
		if d1[lineplusoffset] ~= nil then
			...
			local r,g,b,img = DPSMate:GetClassColor(DPSMate.Cache.DPSMateUser[user][2])
...
function DPSMate.Modules.DetailsHealing:PlayerSpells_Update(i, comp)
	...
	for line=1,10 do
		...
		if d2[obj.id][2][lineplusoffset] ~= nil then
			...
			_G(path.."_ScrollButton"..line.."_Value"):SetText(d2[obj.id][3][lineplusoffset][1].." ("..strformat("%.1f", (d2[obj.id][3][lineplusoffset][1]*100/d2[obj.id][1])).."%)")
...
function DPSMate.Modules.DetailsHealing:UpdatePie(gg, cname)
	...
	for cat, val in uArr do
		local percent = (db[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]][val][1]*100/dTot)
...
function DPSMate.Modules.DetailsHealing:UpdateStackedGraph(gg, comp, cname)
	...
	local temp2 = {}
	if toggle3 then
		for cat, val in db2[d1[d4]][DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]] do
			if cat~="i" and val["i"] then
				for c, v in val["i"] do
		...
		for cat, val in db[DPSMate.Cache.DPSMateUser[cname or DetailsUser][1]] do
			if cat~="i" and val["i"] then
				local temp = {}
				for c, v in val["i"] do
					local key = tonumber(strformat("%.1f", c))
...
function DPSMate.Modules.DetailsHealing:CreateGraphTable(comp)
	...
	for i=1, 8 do
		-- Horizontal
		lines[i] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."Healing_LogDetails_Healing"), 5, 270-i*30, 376, 270-i*30, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
		lines[i]:Show()
	end
	-- Vertical
	lines[9] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."Healing_LogDetails_Healing"), 57, 265, 57, 2, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[9]:Show()
	
	lines[10] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."Healing_LogDetails_Healing"), 192, 265, 192, 2, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[10]:Show()
	
	lines[11] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."Healing_LogDetails_Healing"), 252, 265, 252, 2, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[11]:Show()
	
	lines[12] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."Healing_LogDetails_Healing"), 312, 265, 312, 2, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[12]:Show()
end
