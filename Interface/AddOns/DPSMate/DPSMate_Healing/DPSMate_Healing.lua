...
DPSMate.Options.Options[1]["args"]["healing"] = {
	...
	get = function() return DPSMate.DPSMateSettings["windows"][DPSMate.Options.Dewdrop:GetOpenedParent().Key]["options"][1]["healing"] end,
...
function DPSMate.Modules.Healing:GetSettingValues(arr, cbt, k,ecbt)
	...
	for cat, val in pairs(sortedTable) do
		...
		if DPSMate.DPSMateSettings["columnshealing"][1] then str[1] = " "..DPSMate:Commas(va, k)..p; strt[2] = DPSMate:Commas(tot, k)..pt end
		if DPSMate.DPSMateSettings["columnshealing"][3] then str[2] = " ("..strformat("%.1f", 100*varea/totr).."%)" end
		if DPSMate.DPSMateSettings["columnshealing"][2] then str[3] = "("..strformat("%.1f", va/cbt)..p..")"; strt[1] = "("..strformat("%.1f", (tot/cbt))..pt..")" end
		if DPSMate.DPSMateSettings["columnshealing"][4] then str[4] = " ("..strformat("%.1f", va/(ecbt[pname] or cbt))..p..")" end
		tinsert(name, pname) 
...
function DPSMate.Modules.Healing:ShowTooltip(user, k)
	if DPSMate.DPSMateSettings["informativetooltips"] then
		local a,b,c = DPSMate.Modules.Healing:EvalTable(DPSMate.Cache.DPSMateUser[user], k)
		local db = DPSMate:GetModeByArr(DPSMate.Cache.DPSMateHealingTaken, k, "THealingTaken")
		local abn, p, i = {}, 1, 1
