...
function DPSMate.Parser:SelfHits(msg)
	...
	for a,b,c,g in strgfind(msg, "You (%a%a?)\it (.+) for (%d+)(.*)") do
		...
		DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, self.player, AbilityAutoAttack, t[1] or 0, t[2] or 1, 0, 0, 0, 0, t[5], b, t[4] or 0, t[3] or 0)
		DB:DamageDone(self.player, AbilityAutoAttack, t[1] or 0, t[2] or 1, 0, 0, 0, 0, t[5], t[3] or 0, t[4] or 0)
		if self.TargetParty[b] then DB:BuildFail(1, b, self.player, AbilityAutoAttack, t[5]);DB:DeathHistory(b, self.player, AbilityAutoAttack, t[5], t[1] or 0, t[2] or 1, 0, 0) end
		return
	end
...
function DPSMate.Parser:SelfMisses(msg)
	...
	for ta in strgfind(msg, "You attack%. (.+) absorbs all the damage%.") do DB:Absorb(AbilityAutoAttack, ta, self.player); return end
	for a,b in strgfind(msg, "You attack%. (.+) (%a-)%.") do 
		...
		DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, self.player, AbilityAutoAttack, 0, 0, 0, t[1] or 0, t[2] or 0, 0, 0, a, t[3] or 0, 0)
		DB:DamageDone(self.player, AbilityAutoAttack, 0, 0, 0, t[1] or 0, t[2] or 0, 0, 0, 0, t[3] or 0)
	end
end

function DPSMate.Parser:SpellSelfDamage(msg)
	...
	for a,b,c,d,e,f in strgfind(msg, "Your (.+) (%a%a?)\its (.+) for (%d+)(.*)\.%s?(.*)") do 
		...
		DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, self.player, a,  t[2] or 0, t[3] or 1, 0, 0, 0, 0, t[1], c, t[4] or 0, 0)
	...
	for a,b,c in strgfind(msg, "Your (.+) was (.-) by (.+)%.") do 
		...
		DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, self.player, a, 0, 0, 0, 0, t[1] or 0, t[3] or 0, 0, c, t[2] or 0, 0)
...
function DPSMate.Parser:PeriodicDamage(msg)
	...
	for a,b,c,d,e,f in strgfind(msg, "(.+) suffers (%d+) (%a-) damage from (.+)'s (.+)%.(.*)") do
		...
		DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, d, e.."(Periodic)", 1, 0, 0, 0, 0, 0, t[1], a, 0, 0)
	...
	for a,b,c,d,e in strgfind(msg, "(.+) suffers (%d+) (%a-) damage from your (.+)%.(.*)") do
		...
		DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, self.player, d.."(Periodic)", 1, 0, 0, 0, 0, 0, t[1], a, 0, 0)
...
function DPSMate.Parser:FriendlyOrHostilePlayerDamage(msg)
	...
	for f,a,b,c,d,e in strgfind(msg, "(.-)'s (.+) (%a%a?)\its (.+) for (%d+)(.*)\.%s?(.*)") do 
		...
		DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, f, a,  t[2] or 0, t[3] or 1, 0, 0, 0, 0, t[1], c, t[4] or 0, 0)
	...
	for f,a,b,c in strgfind(msg, "(.-)'s (.+) was (.-) by (.+)%.") do 
		t = {false,false,false}
		if b=="dodged" then t[1]=1 elseif b=="blocked" then t[2]=1 else t[3]=1 end
		DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, f, a, 0, 0, 0, 0, t[1] or 0, t[3] or 0, 0, c, t[2] or 0, 0)
...
function DPSMate.Parser:FriendlyOrHostilePlayerHits(msg)
	...
	for a,b,c,d,g in strgfind(msg, "(.-) (%a%a?)\its (.+) for (%d+)(.*)") do
		...
		if c=="you" then c=self.player end
		DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, a, AbilityAutoAttack, t[3] or 0, t[4] or 1, 0, 0, 0, 0, t[5], c, t[2] or 0, t[1] or 0)
		DB:DamageDone(a, AbilityAutoAttack, t[3] or 0, t[4] or 1, 0, 0, 0, 0, t[5], t[1] or 0, t[2] or 0)
		if self.TargetParty[c] then 
			if self.TargetParty[a] then
				DB:BuildFail(1, c, a, AbilityAutoAttack, t[5]);
			end
			DB:DeathHistory(c, a, AbilityAutoAttack, t[5], t[3] or 0, t[4] or 1, 0, 0) end
		return
	end
...
function DPSMate.Parser:FriendlyPlayerMisses(msg)
	...
	for c,ta in strgfind(msg, "(.+) attack\s?%. (.+) absorb\s? all the damage%.") do if ta=="You" then ta=self.player end; DB:Absorb(AbilityAutoAttack, ta, c); return end
	for a,b,c in strgfind(msg, "(.-) attacks%. (.+) (%a-)%.") do 
		...
		if b=="You" then b=self.player end
		DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, a, AbilityAutoAttack, 0, 0, 0, t[1] or 0, t[2] or 0, 0, 0, b, t[3] or 0, 0)
		DB:DamageDone(a, AbilityAutoAttack, 0, 0, 0, t[1] or 0, t[2] or 0, 0, 0, 0, t[3] or 0)
		return
	end
end
...
function DPSMate.Parser:SpellDamageShieldsOnSelf(msg)
	for a,b,c in strgfind(msg, "You reflect (%d+) (%a-) damage to (.+)%.") do 
		t = {tnbr(a)}
		if c == "you" then c=self.player end
		DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, self.player, "Reflection", 1, 0, 0, 0, 0, 0, t[1], c, 0, 0)
...
function DPSMate.Parser:SpellDamageShieldsOnOthers(msg)
	for a,b,c,d in strgfind(msg, "(.+) reflects (%d+) (%a-) damage to (.+)%.") do
		...
		if strfind(a, "%s") then
			DB:EnemyDamage(false, DPSMate.Cache.DPSMateEDD, d, "Reflection", 1, 0, 0, 0, 0, 0, t[1], a, 0, 0)
			DB:DamageTaken(d, "Reflection", 1, 0, 0, 0, 0, 0, t[1], a, 0, 0)
			DB:DeathHistory(d, a, "Reflection", t[1], 1, 0, 0, 0)
		else
			DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, a, "Reflection", 1, 0, 0, 0, 0, 0, t[1], d, 0, 0)
...
function DPSMate.Parser:CreatureVsSelfHits(msg)
	for a,b,c,d in strgfind(msg, "(.+) (%a%a?)\its you for (%d+)(.*)") do
		...
		DB:EnemyDamage(false, DPSMate.Cache.DPSMateEDD, self.player, AbilityAutoAttack, t[1] or 0, t[2] or 1, 0, 0, 0, 0, t[5], a, t[4] or 0, t[3] or 0)
		DB:DamageTaken(self.player, AbilityAutoAttack, t[1] or 0, t[2] or 1, 0, 0, 0, 0, t[5], a, t[3] or 0, t[4] or 0)
		DB:DeathHistory(self.player, a, AbilityAutoAttack, t[5], t[1] or 0, t[2] or 1, 0, t[3] or 0)
		return
	end
end

function DPSMate.Parser:CreatureVsSelfMisses(msg)
	for ta in strgfind(msg, "(.+) attacks%. You absorb all the damage%.") do DB:Absorb(AbilityAutoAttack, self.player, ta); return end
...
function DPSMate.Parser:CreatureVsSelfSpellDamage(msg)
	for a,b,c,d,e in strgfind(msg, "(.+)'s (.+) (%a%a?)\its you for (%d+)(.*)") do
		...
		DB:EnemyDamage(false, DPSMate.Cache.DPSMateEDD, self.player, b, t[1] or 0, t[2] or 1, 0, 0, 0, 0, t[3], a, t[4] or 0, 0)
...
function DPSMate.Parser:PeriodicSelfDamage(msg)
	for a,b,c,d,e in strgfind(msg, "You suffer (%d+) (%a+) damage from (.+)'s (.+)%.(.*)") do -- Potential to track school and resisted damage
		t = {tnbr(a)}
		DB:EnemyDamage(false, DPSMate.Cache.DPSMateEDD, self.player, d.."(Periodic)", 1, 0, 0, 0, 0, 0, t[1], c, 0, 0)
	...
	for a,b,d,e in strgfind(msg, "You suffer (%d+) (%a+) damage from your (.+)%.(.*)") do -- Potential to track school and resisted damage
		t = {tnbr(a)}
		DB:EnemyDamage(false, DPSMate.Cache.DPSMateEDD, self.player, d.."(Periodic)", 1, 0, 0, 0, 0, 0, t[1], self.player, 0, 0)
...
function DPSMate.Parser:CreatureVsCreatureHits(msg) 
	for a,b,c,d,e in strgfind(msg, "(.+) (%a%a?)\its (.+) for (%d+)(.*)") do
		...
		DB:EnemyDamage(false, DPSMate.Cache.DPSMateEDD, c, AbilityAutoAttack, t[1] or 0, t[2] or 1, 0, 0, 0, 0, t[5], a, t[4] or 0, t[3] or 0)
		DB:DamageTaken(c, AbilityAutoAttack, t[1] or 0, t[2] or 1, 0, 0, 0, 0, t[5], a, t[3] or 0, t[4] or 0)
		DB:DeathHistory(c, a, AbilityAutoAttack, t[5], t[1] or 0, t[2] or 1, 0, t[3] or 0)
		return
	end
end

function DPSMate.Parser:CreatureVsCreatureMisses(msg)
	for c, ta in strgfind(msg, "(.+) attacks%. (.+) absorbs all the damage%.") do DB:Absorb(AbilityAutoAttack, ta, c); return end
	for a,b,c in strgfind(msg, "(.+) attacks%. (.-) (.+)%.") do 
		t = {false,false,false}
		if c=="parries" then t[1]=1 elseif c=="dodges" then t[2]=1 else t[3]=1 end 
		DB:EnemyDamage(false, DPSMate.Cache.DPSMateEDD, b, AbilityAutoAttack, 0, 0, 0, t[1] or 0, t[2] or 0, 0, 0, a, t[3] or 0, 0)
		DB:DamageTaken(b, AbilityAutoAttack, 0, 0, 0, t[1] or 0, t[2] or 0, 0, 0, a, 0, t[3] or 0)
		return
	end
	for a,b in strgfind(msg, "(.+) misses (.+)%.") do 
...
function DPSMate.Parser:SpellPeriodicDamageTaken(msg)
	for a,b,c,d,e,f in strgfind(msg, "(.+) suffers (%d+) (%a+) damage from (.+)'s (.+)%.(.*)") do
		t = {tnbr(b)}
		DB:EnemyDamage(false, DPSMate.Cache.DPSMateEDD, a, e.."(Periodic)", 1, 0, 0, 0, 0, 0, t[1], d, 0, 0)
...
function DPSMate.Parser:CreatureVsCreatureSpellDamage(msg)
	for a,b,c,d,e,f in strgfind(msg, "(.+)'s (.+) (%a%a?)\its (.+) for (%d+)(.*)") do
		...
		DB:EnemyDamage(false, DPSMate.Cache.DPSMateEDD, d, b, t[1] or 0, t[2] or 1, 0, 0, 0, 0, t[3], a, t[4] or 0, 0)
...
function DPSMate.Parser:SpellSelfBuff(msg)
	for a,b,c in strgfind(msg, "Your (.+) critically heals (.+) for (%d+)%.") do
		...
		DB:HealingTaken(0, DPSMate.Cache.DPSMateHealingTaken, t[1] or b, a, 0, 1, t[2], self.player)
		DB:HealingTaken(1, DPSMate.Cache.DPSMateEHealingTaken, t[1] or b, a, 0, 1, t[2]-overheal, self.player)
		DB:Healing(0, DPSMate.Cache.DPSMateEHealing, self.player, a, 0, 1, t[2]-overheal, t[1] or b)
		if overheal>0 then 
			DB:Healing(2, DPSMateOverhealing, self.player, a, 0, 1, overheal, t[1] or b)
			DB:HealingTaken(2, DPSMate.Cache.DPSMateOverhealingTaken, t[1] or b, a, 0, 1, overheal, self.player)
		end
		DB:Healing(1, DPSMate.Cache.DPSMateTHealing, self.player, a, 0, 1, t[2], t[1] or b)
	...
	for a,b,c in strgfind(msg, "Your (.+) heals (.+) for (%d+)%.") do
		...
		if b=="you" then t[1]=self.player end
		overheal = self:GetOverhealByName(t[2], t[1] or b)
		DB:HealingTaken(0, DPSMate.Cache.DPSMateHealingTaken, t[1] or b, a, t[4], t[3], t[2], self.player)
		DB:HealingTaken(1, DPSMate.Cache.DPSMateEHealingTaken, t[1] or b, a, t[4], t[3], t[2]-overheal, self.player)
		DB:Healing(0, DPSMate.Cache.DPSMateEHealing, self.player, a, t[4], t[3], t[2]-overheal, t[1] or b)
		if overheal>0 then 
			DB:Healing(2, DPSMateOverhealing, self.player, a, t[4], t[3], overheal, t[1] or b) 
			DB:HealingTaken(2, DPSMate.Cache.DPSMateOverhealingTaken, t[1] or b, a, t[4], t[3], overheal, self.player)
		end
		DB:Healing(1, DPSMate.Cache.DPSMateTHealing, self.player, a, t[4], t[3], t[2], t[1] or b)
...
function DPSMate.Parser:SpellPeriodicSelfBuff(msg)
	for a,b,c in strgfind(msg, "You gain (%d+) health from (.+)'s (.+)%.") do
		...
		DB:HealingTaken(0, DPSMate.Cache.DPSMateHealingTaken, self.player, c.."(Periodic)", 1, 0, t[1], b)
		DB:HealingTaken(1, DPSMate.Cache.DPSMateEHealingTaken, self.player, c.."(Periodic)", 1, 0, t[1]-overheal, b)
		DB:Healing(0, DPSMate.Cache.DPSMateEHealing, b, c.."(Periodic)", 1, 0, t[1]-overheal, self.player)
		if overheal>0 then 
			DB:Healing(2, DPSMateOverhealing, b, c.."(Periodic)", 1, 0, overheal, self.player)
			DB:HealingTaken(2, DPSMate.Cache.DPSMateOverhealingTaken, self.player, c.."(Periodic)", 1, 0, overheal, b)
		end
		DB:Healing(1, DPSMate.Cache.DPSMateTHealing, b, c.."(Periodic)", 1, 0, t[1], self.player)
	...
	for a,b in strgfind(msg, "You gain (%d+) health from (.+)%.") do 
		t = {tnbr(a)}
		overheal = self:GetOverhealByName(t[1], self.player)
		DB:HealingTaken(0, DPSMate.Cache.DPSMateHealingTaken, self.player, b.."(Periodic)", 1, 0, t[1], self.player)
		DB:HealingTaken(1, DPSMate.Cache.DPSMateEHealingTaken, self.player, b.."(Periodic)", 1, 0, t[1]-overheal, self.player)
		DB:Healing(0, DPSMate.Cache.DPSMateEHealing, self.player, b.."(Periodic)", 1, 0, t[1]-overheal, self.player)
		if overheal>0 then 
			DB:Healing(2, DPSMateOverhealing, self.player, b.."(Periodic)", 1, 0, overheal, self.player)
			DB:HealingTaken(2, DPSMate.Cache.DPSMateOverhealingTaken, self.player, b.."(Periodic)", 1, 0, overheal, self.player)
		end
		DB:Healing(1, DPSMate.Cache.DPSMateTHealing, self.player, b.."(Periodic)", 1, 0, t[1], self.player)
...
function DPSMate.Parser:SpellPeriodicFriendlyOrHostilePlayerBuffs(msg)
	for f,a,b,c in strgfind(msg, "(.+) gains (%d+) health from (.+)'s (.+)%.") do
		...
		DB:HealingTaken(0, DPSMate.Cache.DPSMateHealingTaken, f, c.."(Periodic)", 1, 0, t[1], b)
		DB:HealingTaken(1, DPSMate.Cache.DPSMateEHealingTaken, f, c.."(Periodic)", 1, 0, t[1]-overheal, b)
		DB:Healing(0, DPSMate.Cache.DPSMateEHealing, b, c.."(Periodic)", 1, 0, t[1]-overheal, f)
		if overheal>0 then 
			DB:Healing(2, DPSMateOverhealing, b, c.."(Periodic)", 1, 0, overheal, f)
			DB:HealingTaken(2, DPSMate.Cache.DPSMateOverhealingTaken, f, c.."(Periodic)", 1, 0, overheal, b)
		end
		DB:Healing(1, DPSMate.Cache.DPSMateTHealing, b, c.."(Periodic)", 1, 0, t[1], f)
	...
	for f,a,b in strgfind(msg, "(.+) gains (%d+) health from your (.+)%.") do 
		t = {tnbr(a)}
		overheal = self:GetOverhealByName(t[1], f)
		DB:HealingTaken(0, DPSMate.Cache.DPSMateHealingTaken, f, b.."(Periodic)", 1, 0, t[1], self.player)
		DB:HealingTaken(1, DPSMate.Cache.DPSMateEHealingTaken, f, b.."(Periodic)", 1, 0, t[1]-overheal, self.player)
		DB:Healing(0, DPSMate.Cache.DPSMateEHealing, self.player, b.."(Periodic)", 1, 0, t[1]-overheal)
		if overheal>0 then 
			DB:Healing(2, DPSMateOverhealing, self.player, b.."(Periodic)", 1, 0, overheal)
			DB:HealingTaken(2, DPSMate.Cache.DPSMateOverhealingTaken, f, b.."(Periodic)", 1, 0, overheal, self.player)
		end
		DB:Healing(1, DPSMate.Cache.DPSMateTHealing, self.player, b.."(Periodic)", 1, 0, t[1])
	...
	for f,a,b in strgfind(msg, "(.+) gains (%d+) health from (.+)%.") do 
		t = {tnbr(a)}
		overheal = self:GetOverhealByName(t[1], f)
		DB:HealingTaken(0, DPSMate.Cache.DPSMateHealingTaken, f, b.."(Periodic)", 1, 0, t[1], f)
		DB:HealingTaken(1, DPSMate.Cache.DPSMateEHealingTaken, f, b.."(Periodic)", 1, 0, t[1]-overheal, f)
		DB:Healing(0, DPSMate.Cache.DPSMateEHealing, f, b.."(Periodic)", 1, 0, t[1]-overheal, f)
		if overheal>0 then 
			DB:Healing(2, DPSMateOverhealing, f, b.."(Periodic)", 1, 0, overheal, f)
			DB:HealingTaken(2, DPSMate.Cache.DPSMateOverhealingTaken, f, b.."(Periodic)", 1, 0, overheal, f)
		end
		DB:Healing(1, DPSMate.Cache.DPSMateTHealing, f, b.."(Periodic)", 1, 0, t[1], f)
...
function DPSMate.Parser:SpellFriendlyOrlHostilePlayerBuff(msg)
	for a,b,c,d in strgfind(msg, "(.-)'s (.+) critically heals (.+) for (%d+)%.") do 
		...
		DB:HealingTaken(0, DPSMate.Cache.DPSMateHealingTaken, t[2] or c, b, 0, 1, t[1], a)
		DB:HealingTaken(1, DPSMate.Cache.DPSMateEHealingTaken, t[2] or c, b, 0, 1, t[1]-overheal, a)
		DB:Healing(0, DPSMate.Cache.DPSMateEHealing, a, b, 0, 1, t[1]-overheal, t[2] or c)
		if overheal>0 then 
			DB:Healing(2, DPSMateOverhealing, a, b, 0, 1, overheal, t[2] or c)
			DB:HealingTaken(2, DPSMate.Cache.DPSMateOverhealingTaken, t[2] or c, b, 0, 1, overheal, a)
		end
		DB:Healing(1, DPSMate.Cache.DPSMateTHealing, a, b, 0, 1, t[1], t[2] or c)
	...
	for a,b,c,d in strgfind(msg, "(.-)'s (.+) heals (.+) for (%d+)%.") do 
		...
		if c=="you" then t[2]=self.player end
		overheal = self:GetOverhealByName(t[1], t[2] or c)
		DB:HealingTaken(0, DPSMate.Cache.DPSMateHealingTaken, t[2] or c, b, t[4], t[3], t[1], a)
		DB:HealingTaken(1, DPSMate.Cache.DPSMateEHealingTaken, t[2] or c, b, t[4], t[3], t[1]-overheal, a)
		DB:Healing(0, DPSMate.Cache.DPSMateEHealing, a, b, t[4], t[3], t[1]-overheal, t[2] or c)
		if overheal>0 then 
			DB:Healing(2, DPSMateOverhealing, a, b, t[4], t[3], overheal, t[2] or c)
			DB:HealingTaken(2, DPSMate.Cache.DPSMateOverhealingTaken, t[2] or c, b, t[4], t[3], overheal, a)
		end
		DB:Healing(1, DPSMate.Cache.DPSMateTHealing, a, b, t[4], t[3], t[1], t[2] or c)
...
function DPSMate.Parser:CreatureVsSelfHitsAbsorb(msg)
	for c, b, a,ka, absorbed in strgfind(msg, "(.+) (%a%a?)\its you for (.+)%.(.*)%((%d+) absorbed%)") do DB:SetUnregisterVariables(tnbr(absorbed), AbilityAutoAttack, c) end
end

function DPSMate.Parser:CreatureVsCreatureHitsAbsorb(msg)
	for c, b, a, d, ka, absorbed in strgfind(msg, "(.+) (%a%a?)\its (.+) for (.+)%.(.*)%((%d+) absorbed%)") do DB:SetUnregisterVariables(tnbr(absorbed), AbilityAutoAttack, c) end
end
...
function DPSMate.Parser:PetSpellDamage(msg)
	...
	for a,b,c in strgfind(msg, "(.-)'s (.+) was resisted by (.+)%.") do 
		if c=="you" then c=self.player end
		DB:EnemyDamage(true, DPSMate.Cache.DPSMateEDT, a, b,  0, 0, 0, 0, 0, 1, 0, c, 0, 0)
