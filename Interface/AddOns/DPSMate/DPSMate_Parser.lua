...
if not DPSMate.DB.has_superwow then
	...
	--DPSMate.Parser:RegisterEvent("RAW_COMBATLOG")
	DPSMate.Parser.RawEvents = {}
end
...
function DPSMate.Parser:OnLoad()
	...
	DPSMate.Cache.DPSMateUser[self.player][8] = UL("player")
...
function DPSMate.Parser:GetUnitByName(target)
	--if strsub(target, 1, 2) == "0x" then target = UnitName(target) end --guid
	local unit = self.TargetParty[target]
	if not unit then
		--wjf test
		if DPSMate.debug and DPSMate.DB.has_superwow then 
			if not debugp then 
				print(target) 
				print(arg1)
				print(arg2)
...
DPSMate.Parser.PLAYER_AURAS_CHANGED = function(unit)
	...
	ToolTipParent = ToolTipParent or UIParent

	for i=1, 4 do
   		DPSMate_Tooltip:SetOwner(ToolTipParent, "ANCHOR_NONE")
		...
		if debuffDispelType and DPSMate.Cache.DPSMateAbility[aura] then
			DPSMate.Cache.DPSMateAbility[aura][2] = debuffDispelType
		end
	end
end
...
DPSMate.Parser.CHAT_MSG_COMBAT_HOSTILEPLAYER_HITS = function(arg1)
	this:FriendlyOrHostilePlayerHits(arg1)
end
...
DPSMate.Parser.CHAT_MSG_SPELL_FRIENDLYPLAYER_DAMAGE = function(arg1)
	this:FriendlyOrHostilePlayerDamage(arg1)
end
...
DPSMate.Parser.RAW_COMBATLOG = function(arg1)
	local func = this.RawEvents[arg1]
	if func then func(arg2) end
end

function DPSMate.Parser:RegisterRawEvent(arg1)
	if not arg1 then 
		self.RawEvents = {} 
		self:UnregisterEvent("CHAT_MSG_SPELL_SELF_BUFF")
	else
		if arg1 == "CHAT_MSG_SPELL_SELF_BUFF" then
			self:RegisterEvent(arg1)
		else
			self.RawEvents[arg1] = self[arg1]
		end
	end
end

DPSMate.Parser:SetScript("OnEvent", function() local func = this[event] if func then func(arg1) end end)