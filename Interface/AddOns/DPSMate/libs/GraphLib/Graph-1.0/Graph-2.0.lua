...
function GraphFunctions:HideTextures()
	local k=table.getn(self.TexturesUsed)
	while k>0 do
		self.Textures[table.getn(self.Textures)+1]=self.TexturesUsed[k]
...
function GraphFunctions:FindTexture()
	local t
	if table.getn(self.Textures)>0 then
		t=self.Textures[table.getn(self.Textures)]
		self.TexturesUsed[table.getn(self.TexturesUsed)+1]=t
		self.Textures[table.getn(self.Textures)]=nil
		return t
	end
	local g=self:CreateTexture(nil,"BACKGROUND")
	self.TexturesUsed[table.getn(self.TexturesUsed)+1]=g
	return g
end
...
function lib:HideLines(C)
	if C.GraphLib_Lines then
		for i = table.getn(C.GraphLib_Lines_Used), 1, -1 do
...
function lib:DrawBar(C, sx, sy, ex, ey, color, level)
	...
	if (table.getn(C.GraphLib_Bars))>0 then
		Bar=C.GraphLib_Bars[table.getn(C.GraphLib_Bars)]
		tremove(C.GraphLib_Bars,table.getn(C.GraphLib_Bars))
		Bar:Show()		

		Tri=C.GraphLib_Tris[table.getn(C.GraphLib_Tris)]
		tremove(C.GraphLib_Tris,table.getn(C.GraphLib_Tris))
		Tri:Show()		
	end
...
function lib:HideBars(C)
	...
	while (table.getn(C.GraphLib_Bars_Used))>0 do
		C.GraphLib_Bars[table.getn(C.GraphLib_Bars)+1]=C.GraphLib_Bars_Used[table.getn(C.GraphLib_Bars_Used)]
		C.GraphLib_Bars[table.getn(C.GraphLib_Bars)]:Hide()
		C.GraphLib_Bars_Used[table.getn(C.GraphLib_Bars_Used)]=nil

		C.GraphLib_Tris[table.getn(C.GraphLib_Tris)+1]=C.GraphLib_Tris_Used[table.getn(C.GraphLib_Tris_Used)]
		C.GraphLib_Tris[table.getn(C.GraphLib_Tris)]:Hide()
		C.GraphLib_Tris_Used[table.getn(C.GraphLib_Tris_Used)]=nil
	end
end
