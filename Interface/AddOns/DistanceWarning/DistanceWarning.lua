...
local function checkTurtle ()
    ...
    return tonumber(minor) >= 16
end

local isCNTurtle = checkTurtle() and GetLocale() == "zhCN"
...
function DistanceWarning_OnEvent(event,arg1)
	...
	if(event=="CHARACTER_POINTS_CHANGED") then
		if(DW_PLAYER_CLASS=="猎人") then
			...
			local tab = isCNTurtle and 3 or 2
			local _,texture,_,_,rank,_,_,_=GetTalentInfo(tab,7);
		...
		elseif(DW_PLAYER_CLASS=="法师") then
			...
			local tab1 = isCNTurtle and 1 or 2
			local tab2 = isCNTurtle and 2 or 3
			local _,texture1,_,_,rank1,_,_,_=GetTalentInfo(tab1,4);
			local _,texture2,_,_,rank2,_,_,_=GetTalentInfo(tab2,11);
		...
		elseif(DW_PLAYER_CLASS=="德鲁伊") then
			...
			local tab = isCNTurtle and 3 or 1
			local _,texture,_,_,rank,_,_,_=GetTalentInfo(tab,10); 
			if(not(texture) or rank==nil) then
				DW_NATUREREACH_RANK=0;
			else 
		...
		elseif(DW_PLAYER_CLASS=="术士") then
			...
			local tab1 = isCNTurtle and 2 or 1
			local tab2 = isCNTurtle and 1 or 3
			local _,texture1,_,_,rank1,_,_,_=GetTalentInfo(tab1,10);
			local _,texture2,_,_,rank2,_,_,_=GetTalentInfo(tab2,10);
			if(not(texture1) or rank1==nil) then
				DW_GRIMREACH_RANK=0;
			else
