...
function update_item_configuration()
	if not selected_item then
        ...
        hide_checkbox:Hide()
        stack_total_bg:Hide()
        stack_total_label:Hide()
        vendor_price_label:Hide()
        price_input_tip:Hide()
    else
        ...
        hide_checkbox:Show()
        stack_total_bg:Show()
        stack_total_label:Show()
        vendor_price_label:Show()
        price_input_tip:Show()
        ...
        do
            ...
            if unit_buyout_price > 0 then
                local stack_total = unit_buyout_price * stack_size
                stack_total_label:SetText("每叠总价：" .. money.to_string(stack_total, nil, nil, aux.color.text.enabled))
            else
                stack_total_label:SetText("每叠总价：无")
            end
        end
