...
function item_seller_column_fill(cell, record, _, _, _, indented)
	cell.icon:Show()
	cell.spacer:Show()
	cell.icon:SetTexture(record.texture)
	if indented then
...
function item_seller_column_cmp(record_a, record_b, desc)
	if item_seller_mode then
...
M.search_columns = {
    {
        ...
        fill = item_seller_column_fill,
        cmp = item_seller_column_cmp,
        isItemSeller = true,
    },
    {
        title = '等级',
...
--            end
    ...
    },
    {
        ...
        align = 'RIGHT',
        fill = function(cell, record)
            cell.text:SetText(record.unit_buyout_price > 0 and money.to_string(ceil(record.unit_buyout_price), true) or '---')
        end,
        cmp = function(record_a, record_b, desc)
            local price_a = record_a.unit_buyout_price > 0 and record_a.unit_buyout_price or (desc and -aux.huge or aux.huge)
            local price_b = record_b.unit_buyout_price > 0 and record_b.unit_buyout_price or (desc and -aux.huge or aux.huge)
            return sort_util.compare(price_a, price_b, desc)
        end,
    },
    {
...
local methods = {
    ...
    OnHeadColumnClick = function()
        local button = arg1
        local rt = this.rt
        local columnIndex = this.columnIndex
        local cellInfo = rt.headCells[columnIndex].info

        if button == 'RightButton' and cellInfo.isPrice then
        ...
        if button == 'LeftButton' and cellInfo.isItemSeller then
            item_seller_mode = not item_seller_mode
            for _, cell in rt.headCells do
                if cell.info.isItemSeller then
                    cell:SetText(cell.info.title[item_seller_mode and 2 or 1])
                end
            end
            rt:SetSort()
            return
        ...
        if getn(rt.sorts) > 0 and rt.sorts[1].index == columnIndex then
            descending = not rt.sorts[1].descending
        end
        rt:SetSort((descending and -1 or 1) * columnIndex)
    end,
...
function M.new(parent, rows, columns)
    ...
    rt:SetScript('OnShow', function()
        for _, cell in this.headCells do
            if cell.info.isPrice then
                cell:SetText(cell.info.title[price_per_unit and 1 or 2])
            elseif cell.info.isItemSeller then
                cell:SetText(cell.info.title[item_seller_mode and 2 or 1])
            end
        end
    end)
    ...
    for i, column in ipairs(rt.columns) do
        ...
        if column.isPrice then
            -- Price column handled in OnShow
        elseif column.isItemSeller then
            cell:SetText(column.title[item_seller_mode and 2 or 1])
        else
