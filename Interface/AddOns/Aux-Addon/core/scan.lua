...
function scan_page(i)
	...
	-- 批量收集和处理所有拍卖（TSM 风格）
	if i == 1 then
		local has_auto_action = false
		local ui_update_counter = 0  -- UI 更新计数器
		
		-- 第一遍：批量处理所有拍卖数据
		for idx = 1, PAGE_SIZE do
			local auction_info = info.auction(idx, get_state().params.type)
			if auction_info and (auction_info.owner or get_state().params.ignore_owner or aux.account_data.ignore_owner) then
				auction_info.index = idx
				...
				-- 使用批量模式收集历史数据（不立即写入）
				history.process_auction_batch(auction_info, pages)
				...
				elseif not get_query().validator or get_query().validator(auction_info) then
					...
					-- UI 更新节流：每 10 条拍卖才触发一次 UI 更新
					ui_update_counter = ui_update_counter + 1
					if ui_update_counter >= 10 and get_state().params.on_page_scanned then
						get_state().params.on_page_scanned()
						ui_update_counter = 0
					end
				end
			end
		...
		-- 批量提交历史数据更新
		history.flush_batch_updates()
		
		-- 如果有自动操作，回退到原始递归模式处理
		if has_auto_action then
			i = 1
		else
			-- 没有自动操作，直接跳到页面结束
			i = PAGE_SIZE + 1
...
-- TSM 风格优化：更激进的超时策略和智能数据检查
function wait_for_list_results()
    local updated, last_update, first_update
    ...
    local timeout = aux.later(2, get_state().last_list_query)  -- TSM 风格：2秒初始超时
	...
	return aux.when(function()
		...
		if last_update and GetTime() - last_update > 1.5 then  -- TSM 风格：1.5秒更新超时
			return true
		end
...
-- TSM 风格：只要大部分数据加载完成即可（80% 阈值）
function owner_data_sufficient()
	local total_count = 0
	local valid_count = 0
