...
function post_auction(slot, k)
	...
	if item_info.item_key == state.item_key and info.auctionable(item_info.tooltip, nil, true) and item_info.aux_quantity == state.stack_size then
		...
		if start_price == 0 then
			...
			local cached_prices = pricing_cache.get_cached_price(state.item_key, item_info)
			
			if cached_prices then
				start_price = cached_prices.suggested_start
				buyout_price = max(buyout_price, cached_prices.suggested_buyout)
				
				-- 检查是否应该分解或出售给商人
				if cached_prices.disenchant_value and buyout_price < cached_prices.disenchant_value * 0.95 - 30 then
					aux.print("autopricing recommends disenchanting!")
					return stop()
				end
				
				if cached_prices.vendor_price > 0 and cached_prices.market_value then
					if cached_prices.market_value < cached_prices.vendor_price * 1.35 then
		...
		aux.event_listener('CHAT_MSG_SYSTEM', function(kill)
			if arg1 == ERR_AUCTION_STARTED then
				...
				send_signal()
				kill()
			elseif arg1 == ERR_AUCTION_DATABASE_ERROR then
				aux.print('拍卖商品到达拍卖堆叠上限')
				kill()
				return stop()
			end
		end)
