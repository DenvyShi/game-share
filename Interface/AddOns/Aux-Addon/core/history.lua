...
-- TSM 风格：批量处理模式 - 只收集数据，不立即写入
function M.process_auction_batch(auction_record, pages)
	...
	if unit_buyout_price > 0 then
		-- 收集最低价格
		if not pending_updates[item_key] or unit_buyout_price < pending_updates[item_key] then
			pending_updates[item_key] = unit_buyout_price
		end
		
		-- 收集需要分享的数据
		if aux.account_data.sharing == true and pages and pages < 15 then
			if not pending_share_data[item_key] or unit_buyout_price < pending_share_data[item_key] then
				pending_share_data[item_key] = unit_buyout_price
			end
		end
	end
end
...
-- 批量提交所有待处理的更新
function M.flush_batch_updates()
	-- 批量写入历史数据
	for item_key, unit_buyout_price in pairs(pending_updates) do
		local item_record = read_record(item_key)
	...
	if GetChannelName("LFT") ~= 0 then
		for item_key, unit_buyout_price in pairs(pending_share_data) do
			ChatThrottleLib:SendChatMessage("BULK", nil, "AuxData," .. item_key .."," .. unit_buyout_price , "CHANNEL", nil, GetChannelName("LFT"))
		end
	end
	
	-- 清空缓冲区
	wipe(pending_updates)
	wipe(pending_share_data)
end
...
function M.process_auction(auction_record, pages)
	...
	if unit_buyout_price > 0 and unit_buyout_price < (item_record.daily_min_buyout or aux.huge) then
		...
		if aux.account_data.sharing == true then
			-- 修复：检查 pages 是否为 nil，避免全扫描时分享数据
