...
CS_ADDONORGANIZER_VERSIONNUMBER = "";
BINDING_HEADER_CS_ADDONORGANIZER_SEP = "AddOnOrganizer";
BINDING_NAME_CS_ADDONORGANIZER_CONFIG = "显示/隐藏";
...
function CS_AddOnOrganizer_ListShowHide()
	...
	if (CS_AddOnOrganizer_List:IsVisible()) then
		...
		CS_AddOnOrganizer_List_Title:SetText ("插件管理器"..CS_ADDONORGANIZER_VERSIONNUMBER);
	...
	for i=1, CS_ADDONORGANIZER_ADDONS_DISPLAYED, 1 do
		...
		if ( addonIndex <= numaddons ) then
			...
			if(CS_AddOnOrganizer_AddOnList[addonIndex] == 1) then
				addonTitleTag:SetText("|CFF00FF00开启|r");
				if(enabled)then
				...
				addonTitleTag:SetText("|CFFFF0000关闭|r");
				if(not enabled)then
					addonTitleTag:SetTextColor(1.0,0.7,0.0);
				else
...
function CS_AddOnOrganizer_TitleButton_OnEnter()
	...
	if(loadondemand) then
		loadondemand = "|CFF00FF00正确|r";
	else
		loadondemand = "|CFFFF0000错误|r";
	end
	
	if(loadable ~= nil)then
		GameTooltip_AddNewbieTip(name, 1.0, 1.0, 1.0, title.."\n"..notes.."\n|CFFFFFFFF插件加载:|r|CFF00FF00正确|r\n".."|CFFFFFFFF延迟加载:|r "..loadondemand.."\n|CFFFFFFFF依赖:|r "..dependencies, 1);
	elseif(reason == "关闭") then
		GameTooltip_AddNewbieTip(name, 1.0, 1.0, 1.0, title.."\n"..notes.."\n|CFFFFFFFF插件加载:|r |CFFFF0000错误|r\n|CFFFFFFFF原因:|r |CFFFF0000关闭|r\n|CFFFFFFFF您可能仍然启用这个插件.|r\n".."|CFFFFFFFF延迟加载:|r "..loadondemand.."\n|CFFFFFFFF依赖:|r "..dependencies, 1);
	else
		GameTooltip_AddNewbieTip(name, 1.0, 1.0, 1.0, title.."\n"..notes.."\n|CFFFFFFFF插件加载:|r|CFFFF0000错误|r\n|CFFFFFFFF原因:|r |CFFFF0000关闭|r\n".."|CFFFFFFFF延迟加载:|r "..loadondemand.."\n|CFFFFFFFF依赖:|r "..dependencies, 1);
	end
end
...
function CS_AddOnOrganizer_SaveProfile()
	...
	if(not(ProfileText == "")) then
		...
		if(not(found))then
			...
			DEFAULT_CHAT_FRAME:AddMessage("|CFF00FF00CS_AddOnOrganizer|r - |CFFFFFFFF"..SaveProfileEditBox:GetText().."|r 已经 |CFF00FF00ADDED|r 到配置文件列表!");
		else
			DEFAULT_CHAT_FRAME:AddMessage("|CFF00FF00CS_AddOnOrganizer|r - |CFFFFFFFF"..SaveProfileEditBox:GetText().."|r 已经 |CFF00FF00MODIFIED|r 在配置文件列表中!");
		end
		...
		DEFAULT_CHAT_FRAME:AddMessage("|CFF00FF00CS_AddOnOrganizer|r - |CFFFF0000你必须为简介写一个名字!|r");
	end
end
