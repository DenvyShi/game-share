...
PallyPower_ClassTexture[0] = "Interface\\AddOns\\PallyPower-TurtleWoW\\Icons\\Warrior";
PallyPower_ClassTexture[1] = "Interface\\AddOns\\PallyPower-TurtleWoW\\Icons\\Rogue";
PallyPower_ClassTexture[2] = "Interface\\AddOns\\PallyPower-TurtleWoW\\Icons\\Priest";
PallyPower_ClassTexture[3] = "Interface\\AddOns\\PallyPower-TurtleWoW\\Icons\\Druid";
PallyPower_ClassTexture[4] = "Interface\\AddOns\\PallyPower-TurtleWoW\\Icons\\Paladin";
PallyPower_ClassTexture[5] = "Interface\\AddOns\\PallyPower-TurtleWoW\\Icons\\Hunter";
PallyPower_ClassTexture[6] = "Interface\\AddOns\\PallyPower-TurtleWoW\\Icons\\Mage";
PallyPower_ClassTexture[7] = "Interface\\AddOns\\PallyPower-TurtleWoW\\Icons\\Warlock";
PallyPower_ClassTexture[8] = "Interface\\AddOns\\PallyPower-TurtleWoW\\Icons\\Shaman";
PallyPower_ClassTexture[9] = "Interface\\AddOns\\PallyPower-TurtleWoW\\Icons\\Pet";
...
function PallyPower_OnLoad()
	...
	--mPrint("|cFFFF8080Relar|r祝福助手TurtleWoW version "..PallyPower_Version.." |cff00FF00加载成功!|r")
end
...
function PallyPower_FormatTime(time)
    ...
    return string.format("%d:%02d", floor(time / 60), time - floor(time / 60) * 60);
end
        ...
        if PallyPower_Assignments[UnitName("player")] then
            ...
            for class = 0, 9 do
                if (assign[class] and assign[class] ~= -1) then
                    ...
                    --Print(LastCast[assign[class] .. class])
...
function PallyPower_ScanSpells()
    local RankInfo = {}
    local i = 1
    local nameTalent, icon, iconx, icony, currRank, maxRank
    ...
    if FiveMinBlessing == true then
		...
		if bless then
            ...
            local tmp_str, _ = string.find(spellName, PallyPower_Greater)
        ...
        if bless then
            local tmp_str, _ = string.find(spellName, PallyPower_Greater)
            for id, name in PallyPower_BlessingID do
				if ((PallyPower_Greater..name == bless) and (tmp_str == 1)) then
					local _, _, rank = string.find(spellRank, PallyPower_RankSearch);
					if not (RankInfo[id] and spellRank < RankInfo[id]["rank"]) then
						RankInfo[id] = {};
						RankInfo[id]["rank"] = rank;
...
function PallyPower_Refresh()
	if PpRefreshAfterClear ~= true then
		mPrint("|cFFFF8080祝福助手|r -- |cffFFFF00正在刷新...|r")
	end
	...
	if PpRefreshAfterClear ~= true then
		mPrint("|cFFFF8080祝福助手|r -- |cff00FF00刷新成功!|r")
	elseif PpRefreshAfterClear == true then
		mPrint("|cFFFF8080祝福助手|r -- |cff00FF00清理成功!|r")
		PpRefreshAfterClear = false
	end
end
...
function PallyPower_Clear(fromupdate, who)
    ...
    for name, skills in PallyPower_Assignments do
        if (PallyPower_CheckRaidLeader(who) or name == who) then
            if name == who then
				mPrint("|cFFFF8080祝福助手|r -- |cffFFFF00清理中...|r")
			else
				if (clearTime + 5) < GetTime() then
					clearTime = GetTime()
					mPrint("|cFFFF8080祝福助手|r -- |cffFFFF00清理请求: |r"..who)
				end
			end
...
function PallyPower_ScanInventory()
    ...
    --mPrint("[PallyPower old count] " .. oldcount, r, g, b, a)
    --mPrint("[PallyPower] " .. PP_Symbols, r, g, b, a)
--    if (AllPallys[UnitName("player")] ~= nil) then
--	  mPrint(AllPallys[UnitName("player")], r, g, b, a)
