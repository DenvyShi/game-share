...
function DruidManaBar:UpdatePosition()
	...
	local barx = 23
	if zUI and zUI_config then
		if zUI_config.unitframes.compactmode == "1" and zUI_config.unitframes.showtdfframethen~="1" then
			barx = 33
		end
	end
	if self.db.profile.moveAbove then
		...
		self.bar:SetPoint("BOTTOMLEFT", 116, barx)
