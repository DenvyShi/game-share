...
function CreateFrames()
	...
	-- text mode
		...
		WHUD_OP_TEXT:SetFont(STANDARD_TEXT_FONT, 33,"THINOUTLINE")
	...
	-- overpower timer
		...
		for i=1,3 do
			...
			_G["WHUD_OP_TIMER"..i]:SetFont(STANDARD_TEXT_FONT, 25,"THINOUTLINE")
		...
		WHUD_OP_TIMER3:SetFont(STANDARD_TEXT_FONT, 40,"OUTLINE")
...
function WHUD_Overpower_OnUpdate()
	if WHUD_VARS.Overpower.enabled then
		if not editmode then
			if WHUD_SPELLINFO["压制"][2] > 0 and cooldown == 0 then
				cooldown = WHUD_SPELLINFO["压制"][2] + 5
			end
...
function WHUD_Overpower_OnEvent(arg1)
	if not editmode then
		if (GetUnitName("target")) then
			if (arg1 == string.format(VSDODGESELFOTHER,GetUnitName("target"))) or (string.find(arg1,"你的") and string.find(arg1,"躲闪了 "..UnitName("target"))) then
				if not UnitPlayerControlled("target") and not WHUD_VARS.Overpower.pve then 
					return 
				else
					WHUD_OP_TIME = GetTime() + 4
					if WHUD_SPELLINFO["压制"][2] > 0 then 
						if WHUD_VARS.Overpower.mode == "text" then
			...
			elseif string.find(arg1,"你的压制") then
				WHUD_OP_TIME = 0
			end
		end
	end
...
function WHUD_Overpower_VarUpdate()
	if WHUD_VARS.Overpower.enabled then
		...
		WHUD_OP:SetFrameStrata(WHUD_VARS.Overpower.strata)
		WHUD_OP:SetAlpha(WHUD_VARS.Overpower.transparency)
		WHUD_OP_TEXT:SetFont(STANDARD_TEXT_FONT, 35*WHUD_VARS.Overpower.scale,"THINOUTLINE")
		WHUD_OP:SetScale(WHUD_VARS.Overpower.scale)
		local ef_scale = WHUD_OP:GetEffectiveScale()
		WHUD_OP:SetPoint("TOPLEFT", "UIParent",WHUD_VARS.Overpower.X/ef_scale,WHUD_VARS.Overpower.Y/ef_scale)
		if WHUD_VARS.Overpower.mode == "text" then
