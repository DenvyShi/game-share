...
local SS = {
	...
	["AutoResponse"]		= "我不理你. (" .. name .. " 插件)",

	["TextOptions"] 		= "屏蔽过滤",
	["TextDuration"]		= "默认屏蔽时间",
	["TextWhisperBlock"]	= "不要让我私聊\n被屏蔽的玩家",
	["TextWhisperUnignore"]	= "解除屏蔽.如果我\n私聊他们",
	["TextAutoResponse"]	= "通知我\n如果被屏蔽的人私聊我\n(只有一次)",
	["TextDebugLog"]		= "调试：在聊天中\n忽略日志操作",

    ["TextInformation"]		= "信息",
	["TextEnabled"]			= "启用",

	["ChatIgnored"]			= "%s 现在被屏蔽了。持续时间： %s.",
	["ChatUnignored"]		= "%s 不再被屏蔽.",
	["ChatBlocked"]			= "您的邮件没有发送，因为屏蔽了 %s.",
	["ChatSelf"]			= "你不能屏蔽自己.",
...
SI_FrameCreateHeader = function(frame, text, fontSize, pad)
	...
	t:SetFont("Fonts\\ARIALN.TTF", fontSize)
...
SI_FrameCreateOption = function(frame, name, desc, pad, onclick)
	...
	ct:SetFont("Fonts\\ARIALN.TTF", 11)
	ct:SetText(desc)
...
SI_FormatTimeNoColor = function(t)
	...
	elseif t == TI_AUTOBLOCK then
		...
		if tt < 0 then
			...
			if tt < 60 then
				tt = math.ceil(tt)
				return tt .. " 分钟" .. _s(tt), "00ffff"
			end

			tt = tt / 60
			if tt < 24 then
				tt = math.ceil(tt)
				return tt .. " 小时" .. _s(tt), "ffff00"
			end

			tt = tt / 24
			tt = math.ceil(tt)
			return tt .. " 天" .. _s(tt), "ff0000"
		end
	end
end
...
SI_MainFrame:SetScript("OnEvent", function()
	if event == "ADDON_LOADED" then
		if string.lower(arg1) == SS.AddonDir then
			...
			SI_Print(string.format("%s %s 加载.", SS.AddonName, SS.AddonVersion))
