...
function Necrosis_OnLoad()
	...
	-- Permet de reprer les sorts lancs
	...
	NecrosisButton:RegisterEvent("CHAT_MSG_SPELL_FAILED_LOCALPLAYER");
	NecrosisButton:RegisterEvent("SPELLCAST_STOP");
	NecrosisButton:RegisterEvent("UNIT_AURA");
	...
	NecrosisButton:RegisterEvent("CHAT_MSG_COMBAT_HOSTILE_DEATH");
...
function Necrosis_OnEvent(event)
	...
	elseif event == "CHAT_MSG_SPELL_SELF_DAMAGE" then
		...
		if NecrosisConfig.CriticalDamageSound then
			for spell, creatureName, damage in string.gfind(arg1, NECROSIS_PARSECRITICALDAMAGE) do
				if NecrosisConfig.Sound then
					PlaySoundFile(NECROSIS_SOUND.CriticalDamage);
					break;
				end
			end
		end
...
function Necrosis_RegisterManagement(RegistrationType)
	if RegistrationType == "IN" then
		...
		NecrosisButton:UnregisterEvent("CHAT_MSG_SPELL_FAILED_LOCALPLAYER");
		...
		NecrosisButton:UnregisterEvent("CHAT_MSG_SPELL_SELF_DAMAGE");
		NecrosisButton:UnregisterEvent("UNIT_AURA");
		NecrosisButton:UnregisterEvent("CHAT_MSG_COMBAT_HOSTILE_DEATH");
        -- added to detect ShadowVulnerability affliction and fade
...
function Necrosis_BuildTooltip(button, type, anchor)
	...
	elseif (type == "Buff") and NecrosisConfig.LastBuff ~= 0 then
		GameTooltip:AddLine(NecrosisTooltipData.LastSpell..NECROSIS_SPELL_TABLE[NecrosisConfig.LastBuff].Name);
	elseif (type == "Curse") and NecrosisConfig.LastCurse ~= 0 then
		GameTooltip:AddLine(NecrosisTooltipData.LastSpell..NECROSIS_SPELL_TABLE[NecrosisConfig.LastCurse].Name);
	elseif (type == "Pet") and NecrosisConfig.LastDemon ~= 0 then
		if(NECROSIS_PET_LOCAL_NAME[(NecrosisConfig.LastDemon - 2)]) then
			if(NecrosisConfig.PetName[(NecrosisConfig.LastDemon - 2)] and NecrosisConfig.PetName[(NecrosisConfig.LastDemon - 2)] ~= " ") then
				GameTooltip:AddLine(NecrosisTooltipData.LastSpell.."|c00FF4444 ["..NECROSIS_PET_LOCAL_NAME[(NecrosisConfig.LastDemon - 2)]..":"..NecrosisConfig.PetName[(NecrosisConfig.LastDemon - 2)].."]|r");
			else
		GameTooltip:AddLine(NecrosisTooltipData.LastSpell..NECROSIS_PET_LOCAL_NAME[(NecrosisConfig.LastDemon - 2)]);
			end
		else
			GameTooltip:AddLine(NecrosisTooltipData.LastSpell..NECROSIS_SPELL_TABLE[NecrosisConfig.LastDemon].Name);
		end
	end
	-- Et hop, affichage !
	GameTooltip:Show();
...
function Necrosis_UpdateIcons()
	...
	if NECROSIS_SPELL_TABLE[15].ID and not DominationUp then
		local start, duration = GetSpellCooldown(NECROSIS_SPELL_TABLE[15].ID, BOOKTYPE_SPELL);
		if start > 0 and duration > 0 then
	...
	if NECROSIS_SPELL_TABLE[43].ID then
		local start2, duration2 = GetSpellCooldown(NECROSIS_SPELL_TABLE[43].ID, BOOKTYPE_SPELL);
		if start2 > 0 and duration2 > 0 then
	...
	if NECROSIS_SPELL_TABLE[42].ID and not AmplifyUp then
		local start3, duration3 = GetSpellCooldown(NECROSIS_SPELL_TABLE[42].ID, BOOKTYPE_SPELL);
		if start3 > 0 and duration3 > 0 then
	...
	-- Coloration du bouton en gris si pas assez de mana
		...
		if SummonAvailable then
			if NECROSIS_SPELL_TABLE[37].ID then
				if NECROSIS_SPELL_TABLE[37].Mana > mana or Soulshards == 0 then
					NecrosisSummonerButton:SetNormalTexture("Interface\\AddOns\\Necrosis\\UI\\TPButton-05");
				else
					NecrosisSummonerButton:SetNormalTexture("Interface\\AddOns\\Necrosis\\UI\\TPButton-01");
				end
			end
		end
		...
		if NECROSIS_SPELL_TABLE[38].ID then
			if NECROSIS_SPELL_TABLE[38].Mana > mana then
				NecrosisBuffMenu6:SetNormalTexture("Interface\\AddOns\\Necrosis\\UI\\Lien-03");
			else
				NecrosisBuffMenu6:SetNormalTexture("Interface\\AddOns\\Necrosis\\UI\\Lien-01");
			end
		end
		if NECROSIS_SPELL_TABLE[43].ID then
...
function Necrosis_ButtonSetup()
	if NecrosisConfig.NecrosisLockServ then
		...
		if (NecrosisConfig.StonePosition[9]) and SummonAvailable then
			ShowUIPanel(NecrosisSummonerButton);
		end
	end
end
	...
	while true do
		...
		if (string.find(subSpellName, NECROSIS_TRANSLATION.Rank)) then
			local found = false;
				...
				local _, _, spellRankAsText = string.find( subSpellName, NECROSIS_TRANSLATION.Rank.." (%d)" );
				local rank = tonumber(spellRankAsText);
			...
			if (string.find(spellName, StoneType[stoneID]))
				...
				local stoneSuffix = string.sub(spellName, string.len(NECROSIS_CREATE[stoneID]) + 1);
					if(GetLocale() == "zhCN" or GetLocale() == "zhTW") then
						stoneSuffix = string.sub(spellName, string.find(spellName, NECROSIS_STONE_CREATE) + string.len(NECROSIS_STONE_CREATE),string.find(spellName, NECROSIS_CREATE[stoneID])-1);
					end
...
-- Fonction pour grer les actions effectues par Necrosis au clic sur un bouton
function Necrosis_UseItem(type,button)
	...
	if (type == "Soulstone") then
		Necrosis_UpdateIcons();
		-- Si le mode = 2 (une pierre dans l'inventaire, pas de pierre utilise)
		-- alors on l'utilise
		if (SoulstoneMode == 2) then
			-- Si un joueur est cibl, sur le joueur (avec message d'alerte)
			-- Si un joueur n'est pas cibl, sur le Dmoniste (sans message)
			...
			-- Maintenant que l'on cre un timer sur la session, nous ne sortons plus d'un RL
			NecrosisRL = false;
			-- Et hop, on remet  jour l'affichage des boutons :)
			Necrosis_UpdateIcons();
		-- si il n'y a pas de pierre d'me dans l'inventaire, alors on cre la pierre d'me de rang le plus grand :)
	...
	elseif (type == "Healthstone") then
		...
		-- soit il n'y en a pas dans l'inventaire, et la pierre de plus haut rang est cre
		else
	...
	elseif (type == "Spellstone") then
		-- Si la pierre est quipe ou dans l'inventaire, un clic droit l'quipe / la dsquipe
		if (SpellstoneMode ~= 1 and button == "RightButton")
			-- Si la pierre n'est pas quipe, le raccourci clavier quipe la pierre
			or (SpellstoneMode == 2 and button == "Binding") then
			Necrosis_SwitchOffHand(type);
		-- Si la pierre est quipe, un clic gauche utilise la pierre	
	...
	elseif (type == "Firestone") then
		-- Si la pierre n'existe pas, elle est cre
		if (FirestoneMode == 1) then
		...
		-- Si la pierre existe, un clic droit l'quipe / la dsquiper
	...
	if NecrosisConfig.NecrosisLockServ then
		...
		for index=1, 9, 1 do
			if NecrosisConfig.StonePosition[index] then
				...
				if index == 9 and SummonAvailable then
					NecrosisSummonerButton:SetPoint("CENTER", "NecrosisButton", "CENTER", ((40 * NBRScale) * cos(NecrosisConfig.NecrosisAngle-indexScale)), ((40 * NBRScale) * sin(NecrosisConfig.NecrosisAngle-indexScale)));
					ShowUIPanel(NecrosisSummonerButton);
					indexScale = indexScale + 36;
				end
			end
...
function Necrosis_ClearAllPoints()
	...
	NecrosisSummonerButton:ClearAllPoints();
...
function Necrosis_NoDrag()
	...
	NecrosisSummonerButton:RegisterForDrag("");
...
function Necrosis_Drag()
	...
	NecrosisSummonerButton:RegisterForDrag("LeftButton");
...
function Necrosis_BuffMenu(button)
	if button == "LeftButton" and NecrosisConfig.LastBuff ~= 0 then
		Necrosis_BuffCast(NecrosisConfig.LastBuff);
		return;
	end
...
function Necrosis_CurseMenu(button)
	if button == "LeftButton" and NecrosisConfig.LastCurse ~= 0 then
		Necrosis_CastSpellOnEnemy(NecrosisConfig.LastCurse,"LeftButton");
		return;
	end
...
function Necrosis_PetMenu(button)
	if button == "LeftButton" and NecrosisConfig.LastDemon ~= 0 then
		Necrosis_PetCast(NecrosisConfig.LastDemon);
		return;
	end
...
function Necrosis_SelectBuffMenuTexture()
	if(NecrosisConfig.LastBuff) then
		NecrosisBuffMenuButton:SetNormalTexture("Interface\\AddOns\\Necrosis\\UI\\"..NECROSIS_SPELL_TABLE[NecrosisConfig.LastBuff].TexturePrefix.."-01");
		NecrosisBuffMenuButton:SetHighlightTexture("Interface\\AddOns\\Necrosis\\UI\\"..NECROSIS_SPELL_TABLE[NecrosisConfig.LastBuff].TexturePrefix.."-02");
	end
end
function Necrosis_SelectPetMenuTexture()
	if(NecrosisConfig.LastDemon) then
		NecrosisPetMenuButton:SetNormalTexture("Interface\\AddOns\\Necrosis\\UI\\"..NECROSIS_SPELL_TABLE[NecrosisConfig.LastDemon].TexturePrefix.."-01");
		NecrosisPetMenuButton:SetHighlightTexture("Interface\\AddOns\\Necrosis\\UI\\"..NECROSIS_SPELL_TABLE[NecrosisConfig.LastDemon].TexturePrefix.."-02");
	end
end
...
function Necrosis_BuffCast(type)
	if(NecrosisConfig.LastBuff ~= type and type)then
		NecrosisConfig.LastBuff = type;
		Necrosis_SelectBuffMenuTexture();
	end
	...
	-- Si le Dmoniste possde la peau de dmon mais pas l'armure dmoniaque
...
function Necrosis_BoundCurseCast()
    Necrosis_CastSpellOnEnemy(NecrosisConfig.LastCurse, SelectedCurseMouse);
end
...
function Necrosis_SetDefaultCurse(type, click)
    	if(NecrosisConfig.LastCurse ~= type and type)then
		NecrosisConfig.LastCurse = type;
		Necrosis_SelectCurseMenuTexture();
	end
    --SelectedCurse = type;
    SelectedCurseMouse = click;
    --Necrosis_SelectCurseMenuTexture();
    local mess = "<lightYellow>"..NECROSIS_SPELL_TABLE[NecrosisConfig.LastCurse].Name;
    if (NECROSIS_SPELL_TABLE[NecrosisConfig.LastCurse].Rank) then
        mess = "<lightYellow>"..NECROSIS_SPELL_TABLE[NecrosisConfig.LastCurse].Name.."(Rank "..NECROSIS_SPELL_TABLE[NecrosisConfig.LastCurse].Rank..")";
    end
...
function Necrosis_SelectCurseMenuTexture()
	if(NecrosisConfig.LastCurse) then
		NecrosisCurseMenuButton:SetNormalTexture("Interface\\AddOns\\Necrosis\\UI\\"..NECROSIS_SPELL_TABLE[NecrosisConfig.LastCurse].TexturePrefix.."-01");
		NecrosisCurseMenuButton:SetHighlightTexture("Interface\\AddOns\\Necrosis\\UI\\"..NECROSIS_SPELL_TABLE[NecrosisConfig.LastCurse].TexturePrefix.."-02");
	end
end
...
-- Gestion des casts du menu des dmons
function Necrosis_PetCast(type, click)
	if(NecrosisConfig.LastDemon ~= type and type)then
		NecrosisConfig.LastDemon = type;
		Necrosis_SelectPetMenuTexture();
	end
...
function Necrosis_ChannelDropDown_Initialize()
	
	local info;
	for i = 1, getn(NECROSIS_CHANNELNAME), 1 do
		info = {
			text = NECROSIS_CHANNELNAME[i];
			func = Necrosis_ChannelDropDownButton_OnClick;
		};
		UIDropDownMenu_AddButton(info);
	end
end

function Necrosis_ChannelDropDownButton_OnClick()
	i = this:GetID();
	UIDropDownMenu_SetSelectedID(ChannelDropDown, i);
	NecrosisConfig.ChannelName = i;
end

function Necrosis_ChannelDropDown_OnShow()
	UIDropDownMenu_Initialize(ChannelDropDown, Necrosis_ChannelDropDown_Initialize);
	UIDropDownMenu_SetSelectedID(ChannelDropDown, NecrosisConfig.ChannelName);
