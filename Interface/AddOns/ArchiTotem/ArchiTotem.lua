...
local totemElements = { "Earth", "Fire", "Water", "Air","Tem" }
...
if not ArchiTotem_Options then
	...
	ArchiTotem_Options["Tem"] = {}
	ArchiTotem_Options["Tem"].max = 1
	ArchiTotem_Options["Tem"].shown = 1
	...
	ArchiTotem_Options["Order"].five = "Tem"
...
if not ArchiTotem_TotemData then
	...
	ArchiTotem_TotemData["ArchiTotemButton_Tem1"] = {}
	ArchiTotem_TotemData["ArchiTotemButton_Tem1"].icon = "Interface\\Icons\\Spell_Shaman_TotemRecall"
	ArchiTotem_TotemData["ArchiTotemButton_Tem1"].name = "Totemic Recall"
	ArchiTotem_TotemData["ArchiTotemButton_Tem1"].duration = 0
	ArchiTotem_TotemData["ArchiTotemButton_Tem1"].cooldown = 0
	ArchiTotem_TotemData["ArchiTotemButton_Tem1"].cooldownstarted = nil
	ArchiTotem_TotemData["ArchiTotemButton_Tem1"].casted = nil
...
function ArchiTotem_OnEvent(event)
	if event == "VARIABLES_LOADED" then
		...
		ArchiTotem_Order(ArchiTotem_Options["Order"].first, ArchiTotem_Options["Order"].second, ArchiTotem_Options["Order"].third, ArchiTotem_Options["Order"].forth, ArchiTotem_Options["Order"].five)
...
function ArchiTotem_SetDirection(dir)
	...
	if dir == "down" then
		...
		TemDurationText:SetPoint("CENTER", ArchiTotemButton_Tem1, "CENTER", 0, 26)
	elseif dir == "up" then
		anchor1 = "BOTTOMLEFT"
		anchor2 = "TOPLEFT"
		EarthDurationText:SetPoint("CENTER", ArchiTotemButton_Earth1, "CENTER", 0, -26)
...
function ArchiTotem_Order(first, second, third, forth,five)
	if first == nil or second == nil or third == nil or forth == nil or five ==nil then
	...
	local firstButton, secondButton, thirdButton, forthButton,fiveButton
	...
	fiveButton = "ArchiTotemButton_" .. strupper(string.sub(five, 1, 1)) .. string.sub(five, 2) .. "1"
	...
	ArchiTotem_Options["Order"].five = strupper(string.sub(five, 1, 1)) .. string.sub(five, 2)
	...
	-- Set the anchor
	_G[fiveButton]:ClearAllPoints()
	-- Clear all anchors, or the buttons will be messed up
	_G[fiveButton]:SetPoint("BOTTOMLEFT", forthButton, "BOTTOMRIGHT")
	-- Set the anchor
end
...
function ArchiTotem_Command(cmd)
	...
	if arg[1] == "set" then
		...
		elseif arg[2] == "Tem" then
			if tonumber(arg[3]) > 0 and tonumber(arg[3]) <= 7 then
				ArchiTotem_Options["Tem"].shown = tonumber(arg[3])
				ArchiTotem_UpdateShown()
				-- ////////////////////////////////////////////////////////////////////
