...
-- 特殊物品
local SPECIAL = set(5462, 9173, 17056, 13347, 11511)
...
do
	...
	function Start()
		...
		timeout = GetTime() + 7
		currentIndex = 1
		modelLength = getTableLength(model)
		complete = false
		Stack()
		f:Show()
	end
	...
	f:SetScript('OnUpdate', function()
		function MoveWithDelay(sources, dst)
			if sourceIndex <= getTableLength(sources) then
				local src = sources[sourceIndex]
				-- 开始移动物品
				Move(src, dst)
			else
				-- print("MoveWithDelay sort")
				SortWithDelay()  -- 继续下一个排序
			end
		end

        function SortWithDelay()
            if currentIndex <= modelLength then
                local dst = model[currentIndex]
                ...
                if dst.targetItem and (dst.item ~= dst.targetItem or dst.count < dst.targetCount) then
					...
					if sources then
						sourceIndex = 1
						MoveWithDelay(sources, dst)  -- 开始第一个移动
					else
						print("empty sort")
						SortWithDelay()  -- 继续下一个排序
					end
		...
		delay = delay - arg1
		if delay <= 0 then
			-- 这个是间隔时间，0.5就是0.5秒，1就是1秒，你想间隔多少秒，就改多少秒
			delay = lag  -- 重置延时
			-- 排序，移动
			SortWithDelay()
		end
