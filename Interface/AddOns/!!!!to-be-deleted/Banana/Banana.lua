...
function Banana_UpdateButton(index)
	...
	if BANANA_ICON[index].IsDeath == 1 and BANANA_GREY_OUT_DEATH == 1 then
        ...
        if BANANA_HIDE_UNUSED_BUTTONS == 1 and not (raidTargetStatus[index].Target) then
            Banana_SetAlpha(BANANA_ICON[index].FrameButton);
        else
...
function Banana_OnEvent()
	if ( event == "PLAYER_ENTERING_WORLD" ) then
		...
		Banana_Print("Banana加载。输入/bb， /bbr， /banana或/bananabar打开配置面板.");

		if not BANANA_SHOW_EXTRA_INFO then
			Banana_Print("第一次启动Banana检测到，显示配置窗口.");
	...
	if BANANA_CLEAR_PLAYER_SYMBOL then
		if BANANA_CLEAR_PLAYER_SYMBOL > 0 then
			...
			if Banana_GetSymbol("PLAYER") then
				Banana_Print("清除自己的符号再试一次"..BANANA_CLEAR_PLAYER_SYMBOL_TRY.." (修复了魔兽世界的bug)");
...
function SpellNameChange(name)

	BANANA_SPELL_NAME = name;
...
function Banana_ButtonOnClick(mousebutton)
	...
	if (not IsControlKeyDown()) and mousebutton == "LeftButton" then 
		if Banana_CanUseSymbols() then
            ...
            Banana_PlayError();
			Banana_Print("只有在队伍或团队中才能使用标记目标."); 
			Banana_Print("输入/bb，/bbr或/bananabar打开配置窗口并隐藏未使用的按钮.");
		end
		return;
	end
	...
	if mousebutton == "RightButton" and  Banana_CanUseSymbols() then 
		Banana_TargetRaidSymbol(index);
		CastSpellByName(BANANA_SPELL_NAME);
	end;
...
if UnitExists("target") and GetRaidTargetIndex("target") ~= index then 
	ClearTarget()
end
  
  
  --Banana_Print("index"..index);
...
function Banana_SetAlpha(frame)
    if BananaConfigFrame:IsVisible() then
        frame:Show();
        frame:SetAlpha(0.16);
    else
...
function Banana_CtRaMainTankUpdateByIndex(mtindex)
    ...
    if getglobal(frameName) == nil then
        Banana_Print("为MT创建符号框架 "..mtindex);
...
function Banana_UpdateDialogFromVariables()
	...
	if BANANA_SPELL_NAME then
		SpellNameEidtBox:SetText(BANANA_SPELL_NAME);
	else
		SpellNameEidtBox:SetText("右键施法需要设置的法术");
	end;
