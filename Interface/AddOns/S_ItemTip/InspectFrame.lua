...
local function OnCharacterShow()
    ...
    if frame then
		...
		--让 StatCompare 自己管理位置，避免冲突
			...
			if StatCompare_SaveVar and not StatCompare_SaveVar["alignFree"] then
...
local function HookInspectFrame()
    ...
    InspectFrame:SetScript("OnShow", function()
		...
		if not S_ItemTip_Settings or not S_ItemTip_Settings.showInspectFrame then
			if S_ItemTip_InspectFrame then
				S_ItemTip_InspectFrame:Hide()
			end
			return
    ...
    InspectFrame:SetScript("OnHide", function()
		...
		if StatCompare_SaveVar and not StatCompare_SaveVar["alignFree"] then
			if StatCompareTargetFrame and StatCompareTargetFrame:IsVisible() then
				StatCompareTargetFrame:ClearAllPoints()
				StatCompareTargetFrame:SetPoint("TOPLEFT", InspectFrame, "TOPRIGHT", 0, -2)
...
local function UpdateInspectFrameVisibility()
    ...
    if S_ItemTip_Settings and S_ItemTip_Settings.showInspectFrame then
...
OnCharacterShow = function()
    if not S_ItemTip_Settings or not S_ItemTip_Settings.showInspectFrame then return end
...
CharacterFrame:SetScript("OnShow", function()
    ...
    if PaperDollFrame:IsVisible() and S_ItemTip_Settings and S_ItemTip_Settings.showInspectFrame then
        OnCharacterShow()
    end
end)
...
if InspectFrame then
    ...
    InspectFrame:SetScript("OnShow", function()
        ...
        if InspectFrame.unit and S_ItemTip_Settings and S_ItemTip_Settings.showInspectFrame then
            if not S_ItemTip_InspectFrame then
                CreateFrame_Delayed()
            end
            local frame = S_ItemTip_InspectFrame
...
SlashCmdList["SITEMTIP"] = function(msg)
    ...
    if command == "inspect" then
        if param == "show" then
			if not S_ItemTip_Settings then
