...
TALENTS_LEARNED = "已学";
TALENTS_PLANNED = "模拟";
SHIFT_CLICK_LEARN_TALENT = "按下Shift学习";
PLANNED_RANK = "模拟等级 %d/%d";
local tp_webdata = true
...
function SpecialTalentFrame_OnLoad()
	...
	if ( tp_webdata ) then
		SPECIAL_TALENT_CLASSES = { Druid=1, Hunter=2, Mage=3, Paladin=4, Priest=5, Rogue=6, Shaman=7, Warlock=8, Warrior=9, };
				...
				-- SpecialTalentUI_ResetTwoButton:SetText(GetTalentTabInfo(2));
				-- SpecialTalentUI_ResetThreeButton:SetText(GetTalentTabInfo(3));
local function InitTalent()
	...
	for t=1, MAX_TALENT_TABS do
		if ( GetTalentTabInfo(t) and not SpecialTalentPlannedSaved[player]["plans"][SpecialTalentPlannedSaved[player]["selectedPlan"]][t] ) then
			...
			SpecialTalentUI_ResetOneButton:SetText(GetTalentTabInfo(t));
		end
	end
end
...
-- 			break;
-- 		end
-- 		--button:SetScript("SpecialTalentButton_OnEnter")
-- 		button:Show();
...
function SpecialTalentFrame_Update()
	...
	local tab = SpecialTalentPlannedSaved[player]["plans"][SpecialTalentPlannedSaved[player]["selectedPlan"]]
	for f=1, MAX_TALENT_TABS do -- for each tab frame
		...
		if not tab[f] then
			InitTalent()
			tab = SpecialTalentPlannedSaved[player]["plans"][SpecialTalentPlannedSaved[player]["selectedPlan"]]
		end
		planned = planned..tab[f].points;
		...
		getglobal("SpecialTalentFrameTabFrame"..f.."SpentPoints"):SetText(CYAN_FONT_COLOR_CODE .. tab[f].points .. "|r :"..RED_FONT_COLOR_CODE..talentTabName.."|r: "..NORMAL_FONT_COLOR_CODE..pointsSpent.."|r");
	...
	plannedText:SetText(planned or "nil");
end
